...
describe('provider linear worker runner', () => {
  ...
  it('builds a full first-turn prompt and a continuation prompt', () => {
    ...
    expect(firstPrompt).toContain(
      'add `--reachability-mode dns-public` only when you need explicit worker-local DNS public-resolution evidence. The default path stays deterministic and the helper fails closed when the permit blocks the mode, when the proof URL is loopback/local-only, or when dns-public lookup yields non-public or unresolved answers.'
    );
