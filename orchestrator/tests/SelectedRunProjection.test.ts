...
describe('SelectedRunProjection', () => {
  ...
  it('refreshes projection proofs when child-lane reservation ledger placeholders exist', async () => {
    ...
    await writeFile(patchArtifactPath, 'diff --git a/docs/PRD-linear-lin-issue-1.md b/docs/PRD-linear-lin-issue-1.md\n', 'utf8');
    await writeFile(
  ...
  it('ignores malformed child-lane proof entries while checking refresh eligibility', async () => {
    const { root, paths } = await createHostPaths((currentRoot) => join(currentRoot, '.runs'));
    const childEnv = {
      repoRoot: root,
      runsRoot: join(root, '.runs'),
      outRoot: join(root, 'out'),
    ...
    await writeFile(
      ...
      JSON.stringify(
        buildProviderLinearWorkerProof({
          ...
          owner_status: 'in_progress',
          workspace_path: root,
          child_lanes: [null, reservedChildLane] as unknown as ProviderLinearWorkerChildLaneRecord[],
          updated_at: '2026-04-17T00:35:00.000Z'
        }),
        null,
        2
