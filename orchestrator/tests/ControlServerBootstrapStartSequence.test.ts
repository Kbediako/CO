...
async function createLifecycleRoot() {
  const root = await mkdtemp(join(tmpdir(), 'control-bootstrap-sequence-'));
...
describe('runControlServerBootstrapStartSequence', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('persists metadata before expiry and bridge startup', async () => {
  ...
  it('treats telegram bridge lifecycle startup failure as non-fatal and warns', async () => {
    const { root, runDir, paths } = await createLifecycleRoot();
    const controlPath = join(runDir, 'control.json');
    const telegramBridgeLifecycle = {
      start: vi.fn(async () => {
    ...
    try {
      await expect(
        runControlServerBootstrapStartSequence(
          {
            paths,
            persistControl: async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('Failed to start Telegram oversight bridge: bridge-boom');
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
