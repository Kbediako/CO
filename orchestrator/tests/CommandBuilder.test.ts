...
describe('CommandBuilder', () => {
  ...
  it('does not attach an unrelated failed command artifact for an explicit stage mismatch', async () => {
    const builder = new CommandBuilder(async () => ({
      success: false,
      notes: ['Run delegation guard: Exited with code 1'],
      manifestPath: '.runs/task/run/manifest.json',
        ...
        commands: [
          {
            id: 'spec-guard',
            status: 'failed',
            error_file: '.runs/task/run/errors/01-spec-guard.json'
          }
        ]
      } as unknown as CliManifest
    }));
    ...
    expect(result.artifacts).toContainEqual({
      path: '.runs/task/run/errors/01-spec-guard.json',
      description: 'Command error artifact (spec-guard)'
    });
  });
