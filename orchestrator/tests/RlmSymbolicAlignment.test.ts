...
import { AlignmentChecker } from '../src/cli/rlm/alignment.js';
...
describe('symbolic alignment integration', () => {
  ...
  it('does not hard-block finalization in enforce mode for confirmation-only gates', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'rlm-symbolic-alignment-'));
    const repoRoot = tempDir;
    const runDir = join(repoRoot, 'rlm');
    ...
    expect(result.state.final?.status).toBe('passed');
    expect(result.state.symbolic_iterations[1]?.alignment?.requires_confirmation).toBe(true);
    expect(result.state.symbolic_iterations[1]?.alignment?.enforcement_blocked).toBe(false);
  });

  it('persists state when alignment finalization throws', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'rlm-symbolic-alignment-'));
    const repoRoot = tempDir;
    const runDir = join(repoRoot, 'rlm');
    ...
    const finalizeSpy = vi
      .spyOn(AlignmentChecker.prototype, 'finalize')
    ...
    try {
      const result = await runSymbolicLoop({
        ...
        alignment: {
          enabled: true,
          enforce: false,
          task_id: 'task-finalize-fail',
          run_id: 'run-finalize-fail',
          thread_id: 'thread-finalize-fail',
          agent_id: 'agent-finalize-fail'
        },
        runPlanner: async () => plans[planIndex++] ?? plans[plans.length - 1],
        runSubcall: async () => 'summary output'
      });
      ...
      expect(result.exitCode).toBe(0);
      expect(result.state.final?.status).toBe('passed');
      expect(result.state.final?.alignment).toBeUndefined();
      await expect(readFile(join(runDir, 'state.json'), 'utf8')).resolves.toContain('"status": "passed"');
    } finally {
