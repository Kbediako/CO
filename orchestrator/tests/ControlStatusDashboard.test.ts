...
describe('control status dashboard', () => {
  ...
  it('maps task_started event to session started in the running event column', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        running: [
          {
    ...
    expect(stripAnsi(frame)).toContain('session started');
    expect(stripAnsi(frame)).not.toContain('task started');
  });

  it('does not render a Dashboard: URL line in the default CO STATUS frame', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset(),
      baseUrl: 'http://127.0.0.1:4100',
      taskId: 'local-mcp',
    ...
    expect(stripAnsi(frame)).not.toContain('Dashboard:');
    expect(stripAnsi(frame)).not.toContain('127.0.0.1:4100');
  });

  it('uses double-pipe separator between Codex and Linear budget in the compact combined row', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        rate_limits: {
          codex: {
    ...
    const rateLimitLine = stripAnsi(frame)
      .split('\n')
      .find((line) => line.startsWith('│ Rate Limits: '));
    expect(rateLimitLine).toContain('||');
    expect(rateLimitLine).not.toContain(' | Linear');
  });

  it('shows percent remaining for a non-exhausted Linear request bucket', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        rate_limits: {
          observed_at: '2026-03-30T01:15:00.000Z',
    ...
    const rateLimitLine = stripAnsi(frame)
      .split('\n')
      .find((line) => line.startsWith('│ Rate Limits: '));
    expect(rateLimitLine).toContain('requests 50%');
    expect(rateLimitLine).not.toContain('/ window');
    expect(rateLimitLine).not.toContain('% / ');
  });

  it('shows resets countdown for an exhausted Linear request bucket', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        rate_limits: {
          observed_at: '2026-03-30T01:15:00.000Z',
    ...
    const rateLimitLine = stripAnsi(frame)
      .split('\n')
      .find((line) => line.startsWith('│ Rate Limits: '));
    expect(rateLimitLine).toContain('requests resets 2m');
    expect(rateLimitLine).not.toContain('0/30');
    expect(rateLimitLine).not.toContain('remaining 0');
  });
});