...
describe('control status dashboard', () => {
  ...
  it('renders runtime unchanged when liveReferenceTime equals referenceTime', () => {
    const referenceTime = new Date('2026-03-30T01:15:00.000Z');
    const frame = renderControlStatusFrame({
      dataset: buildDataset(),
      baseUrl: 'http://127.0.0.1:4100',
      taskId: 'local-mcp',
      ...
      throughputTps: 0,
      referenceTime,
      liveReferenceTime: referenceTime
    });

    expect(stripAnsi(frame)).toContain('│ Runtime: 15m 12s');
  });

  it('advances compact-mode runtime by the live reference time delta', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset(),
      baseUrl: 'http://127.0.0.1:4100',
      taskId: 'local-mcp',
      ...
      pendingUpdate: false,
      referenceTime: new Date('2026-03-30T01:15:00.000Z'),
      liveReferenceTime: new Date('2026-03-30T01:15:10.000Z')
    });

    expect(stripAnsi(frame)).toContain('│ Status: 1/4 max allowed | 15m 22s | next 15s');
  });

  it('uses display_event dash sentinel as a signal to fall through to last_message', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        running: [
          {
            ...
            display_event: '-',
            last_event: 'turn_running',
            last_message: 'Executing provider worker step.',
            last_event_at: '2026-03-30T01:14:59.000Z'
          }
        ]
      }),
    ...
    expect(stripAnsi(frame)).toContain('Executing provider worker step');
  });

  it('shows max allowed as n/a when max_allowed is zero', () => {
    const frame = renderControlStatusFrame({
      dataset: buildDataset({
        counts: { ...buildDataset().counts, max_allowed: 0 }
      }),
      baseUrl: 'http://127.0.0.1:4100',
      taskId: 'local-mcp',
      runId: 'control-host',
    ...
    expect(stripAnsi(frame)).toContain('│ Agents: 1/n/a max allowed');
  });

  it('shows max allowed as n/a when max_allowed is undefined', () => {
    const countsWithoutMaxAllowed: OperatorDashboardDataset['counts'] = {
      running: 1,
      retrying: 0,
      issues: 1
    };
    const frame = renderControlStatusFrame({
      dataset: buildDataset({ counts: countsWithoutMaxAllowed }),
      baseUrl: 'http://127.0.0.1:4100',
      taskId: 'local-mcp',
      runId: 'control-host',
      runDir: '/repo/.runs/local-mcp/cli/control-host',
