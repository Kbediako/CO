...
describe('symbolic rlm loop', () => {
  ...
  it('persists deliberation artifacts when logging is enabled', async () => {
    ...
    const result = await runSymbolicLoop({
      ...
      deliberation: {
        ...
        logArtifacts: true,
        run: async () =>
          'Decision focus: keep validating context\nRisks: stale context\nContext gaps: none\nPlanner directives: run one targeted subcall'
      }
    });

    expect(result.exitCode).toBe(0);
    const artifacts = result.state.symbolic_iterations[0]?.deliberation?.artifact_paths;
    expect(artifacts?.prompt).toBeTruthy();
    expect(artifacts?.output).toBeTruthy();
    expect(artifacts?.meta).toBeTruthy();
    await expect(readFile(join(repoRoot, artifacts?.prompt ?? ''), 'utf8')).resolves.toContain('Goal:');
    await expect(readFile(join(repoRoot, artifacts?.output ?? ''), 'utf8')).resolves.toContain('Decision focus:');
    await expect(readFile(join(repoRoot, artifacts?.meta ?? ''), 'utf8')).resolves.toContain('"iteration": 1');
  });
  ...
  it('records deliberation errors but continues the symbolic loop', async () => {
    ...
    expect(artifacts?.meta).toBeTruthy();
    await expect(readFile(join(repoRoot, artifacts?.prompt ?? ''), 'utf8')).resolves.toContain('Goal:');
    await expect(readFile(join(repoRoot, artifacts?.output ?? ''), 'utf8')).resolves.toBe('');
    await expect(readFile(join(repoRoot, artifacts?.meta ?? ''), 'utf8')).resolves.toContain(
      '"error": "deliberation unavailable"'
    );
    expect(result.state.symbolic_iterations[1]?.deliberation?.status).toBe('skipped');
  ...
  it('resolves final_var from a prior subcall output_var binding', async () => {
    ...
    const result = await runSymbolicLoop({
      ...
      deliberation: {
        ...
        run: async (_prompt, meta) => {
          deliberationCalls.push(meta);
          return 'Decision focus: resolve final_var correctly\nRisks: missing output_var mapping\nContext gaps: none\nPlanner directives: keep output_var in summaries';
        }
      }
    });
