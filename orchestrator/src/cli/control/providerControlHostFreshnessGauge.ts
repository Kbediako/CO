...
function evaluateRefreshAge(
  ...
  if (!latest) {
    return metric<number>(null, 'ms', 'unknown', null, null, 'no refresh timestamp observed');
  }
...
function evaluateLinearBudget(
  ...
  if (budget.cooldown_active === true || normalizeOptionalString(budget.suppression) !== 'none') {
    findings.push({
