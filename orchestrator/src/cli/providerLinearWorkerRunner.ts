...
const PROVIDER_LINEAR_GOAL_EVIDENCE_WORKPAD_OBJECTIVE_MAX_CHARS = 96;
...
function extractProviderWorkerRuntimeReportedModel(
  ...
  const reasoningEffort = normalizeProviderWorkerReasoningEffortValue(
    readFirstProviderWorkerStringAtPaths(input, [
      ...
      ['reasoningEffort'],
      ['effort'],
      ['collaboration_mode', 'settings', 'model_reasoning_effort'],
      ['collaboration_mode', 'settings', 'modelReasoningEffort'],
      ['collaboration_mode', 'settings', 'reasoning_effort'],
      ['collaboration_mode', 'settings', 'reasoningEffort'],
      ...
      ['payload', 'effort'],
      ['payload', 'collaboration_mode', 'settings', 'model_reasoning_effort'],
      ['payload', 'collaboration_mode', 'settings', 'modelReasoningEffort'],
      ['payload', 'collaboration_mode', 'settings', 'reasoning_effort'],
      ['payload', 'collaboration_mode', 'settings', 'reasoningEffort'],
      ['payload', 'collaboration_mode', 'settings', 'effort'],
      ...
      ['params', 'turn', 'effort'],
      ['params', 'turn', 'collaboration_mode', 'settings', 'model_reasoning_effort'],
      ['params', 'turn', 'collaboration_mode', 'settings', 'modelReasoningEffort'],
...
function buildPreReviewHandoffGateSection(): string[] {
  return [
    ...
    '- Run the standalone review first. When manifest-backed evidence matters, use the wrapper-led review path by default; if review tooling is unavailable or stalls without a concrete verdict, do not hand off to review state unless a break-glass waiver is recorded with owner, trigger, introduced date, review date, maximum lifetime or expiry, removal condition, reason, and validation evidence. Only after that waiver may you do a manual correctness/regressions/missing-tests review plus a manual elegance checklist as the fallback evidence.',
...
function buildReviewOutcomeGuidanceSection(): string[] {
  return [
    ...
    '- When `review/telemetry.json` reports `review_verdict: unknown` or omits `review_verdict`, treat the semantic verdict as a review-handoff blocker until a fresh review produces `review_verdict: clean` or an explicit break-glass waiver records owner, trigger, introduced date, review date, maximum lifetime or expiry, removal condition, reason, and validation evidence.',
    '- When `review/telemetry.json` reports `status: succeeded` with `review_outcome: bounded-success`, record that wrapper execution completed with a successful bounded review outcome, not as a wrapper blocker or generic quiet-tail failure; this does not override `review_verdict`, and clean handoff still requires `review_verdict: clean`. If you retain a legacy succeeded payload path with a preserved `termination_boundary`, treat it as retained-fallback behavior and require owner, trigger, introduced date, review date, maximum lifetime or expiry, removal condition, reason, and validation evidence before accepting it as bounded review completion.',
...
function extractProviderWorkerGoalEvidence(
  ...
  return buildProviderLinearGoalEvidence({
    ...
    elapsedSeconds: normalizeNonNegativeInteger(
...
function normalizeProviderLinearGoalEvidenceForProof(input: {
  ...
  const base =
    ...
    buildProviderLinearGoalEvidence({
      featureAvailable:
        featureEnabled === false ? false : runtimeMode === 'appserver' ? featureEnabled ?? null : false,
      featureEnabled,
      captureMode:
      ...
      reason:
        featureEnabled === false
          ? 'goals_feature_disabled'
          : runtimeMode === 'appserver'
            ? 'goal_state_not_observed'
            : 'appserver_goal_surface_unavailable'
    });
  ...
  if (goalThreadId && proofThreadId && goalThreadId !== proofThreadId) {
    ...
    !input.candidate &&
    captureMode === 'captured' &&
    compareIsoTimestamp(base.capture_timestamp, input.currentTurnStartedAt) < 0
  ) {
    captureMode = 'stale';
  ...
  return {
    ...base,
    feature_available:
      base.feature_available ?? (featureEnabled === false ? false : runtimeMode === 'appserver' ? true : false),
...
export function formatProviderLinearGoalEvidenceWorkpadSummary(input: {
  goalEvidence: ProviderLinearGoalEvidence | null | undefined;
  manifestPath: string | null | undefined;
}): string {
  const manifestPath = normalizeOptionalString(input.manifestPath) ?? '<manifest unavailable>';
  const evidence = input.goalEvidence ?? null;
  if (!evidence) {
    return `Advisory goal evidence: unavailable; manifest=${manifestPath}; authority=advisory_only; Linear remains source of truth.`;
  }
  const mode = evidence.capture_mode;
  const status = evidence.status ?? 'unknown';
  const objective = evidence.objective
    ? ` objective=${JSON.stringify(formatProviderLinearGoalEvidenceWorkpadObjective(evidence.objective))}`
    : '';
  const thread = evidence.thread_id ?? 'unknown-thread';
  const turn = evidence.turn_id ?? 'unknown-turn';
  const reason = evidence.reason ? ` reason=${evidence.reason}` : '';
  return (
    `Advisory goal evidence: mode=${mode} status=${status} thread=${thread} turn=${turn}` +
    `${objective}${reason}; manifest=${manifestPath}; authority=advisory_only; Linear remains source of truth.`
  );
}

function formatProviderLinearGoalEvidenceWorkpadObjective(objective: string): string {
  const compact = objective.replace(/\s+/gu, ' ').trim();
  if (compact.length <= PROVIDER_LINEAR_GOAL_EVIDENCE_WORKPAD_OBJECTIVE_MAX_CHARS) {
    return compact;
  }
  const head = compact
    .slice(0, PROVIDER_LINEAR_GOAL_EVIDENCE_WORKPAD_OBJECTIVE_MAX_CHARS - 3)
    .trimEnd();
  return `${head}...`;
}
...
async function hydrateProviderLinearWorkerProofFromSessionLog(
  ...
  const hydratedProof: ProviderLinearWorkerProof = {
    ...
    goal_evidence:
      parseState.goalEvidence === null && proof.goal_evidence === undefined
        ? undefined
        : normalizeProviderLinearGoalEvidenceForProof({
            candidate: parseState.goalEvidence ?? null,
            previous: liveScopeChanged ? null : proof.goal_evidence ?? null,
...
async function readProviderWorkerManifestPatchBase(
  manifestPath: string,
  ...
  try {
    ...
    // Fall back to the in-memory snapshot when the manifest is temporarily unreadable.
  }
...
async function patchProviderWorkerGoalEvidenceManifestFile(input: {
  ...
  const manifest = await readProviderWorkerManifestPatchBase(input.manifestPath, input.fallbackManifest);
    ...
    for (let turnNumber = 1; turnNumber <= context.maxTurns; turnNumber += 1) {
      ...
      const queueLiveProofWrite = (): void => {
        ...
        const nextProof: ProviderLinearWorkerProof = {
          ...
          goal_evidence: normalizeProviderLinearGoalEvidenceForProof({
            ...
            previous: liveScopeChanged ? null : finalProof.goal_evidence ?? null,
            proofThreadId: liveThreadId,
            proofTurnId: liveTurnId,
            observedAt: deps.now(),
      ...
      finalProof = {
        ...
        goal_evidence: normalizeProviderLinearGoalEvidenceForProof({
          ...
          previous: parsedScopeChanged ? null : finalProof.goal_evidence ?? null,
