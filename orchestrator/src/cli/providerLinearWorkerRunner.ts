...
function buildReviewOutcomeGuidanceSection(): string[] {
  return [
    ...
    '- When `review/telemetry.json` reports `contract_mode: enforce`, treat `contract_validation.status` other than `valid`, a missing/empty `contract_overall_verdict`, `contract_overall_verdict` of `findings`, `blocked`, or `unknown`, or a missing `review/contract.json` as a review-handoff blocker even if legacy prose parsing would look clean.',
    '- Keep `code_change_proposals[]` and `agent_loop_proposals[]` separate in governed review evidence. Code proposals are reviewer suggestions only; agent-loop proposals should route to typed Linear follow-ups by default and must not be mixed into code findings unless the governed contract marks them blocking.',
...
export function buildProviderWorkerPrompt(
  ...
  if (turnNumber > 1 || continueResidentSessionOnBoot) {
    return [
      ...
      `- Provider docs packet task keys and docs-review retry task ids must stay rooted at the registered provider issue task key \`linear-${issue.id}\`; use \`linear-${issue.id}-docs-packet\` and \`linear-${issue.id}-docs-review\`, not generated suffix keys such as \`linear-${issue.id}-source-freshness-recheck\`.`,
