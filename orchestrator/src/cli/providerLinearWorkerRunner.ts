...
function buildReviewOutcomeGuidanceSection(): string[] {
  return [
    ...
    '- When `review/telemetry.json` reports `contract_mode: enforce`, treat `contract_validation.status` other than `valid`, `contract_overall_verdict` of `findings`, `blocked`, or `unknown`, or a missing `review/contract.json` as a review-handoff blocker even if legacy prose parsing would look clean.',
