...
function buildReviewOutcomeGuidanceSection(): string[] {
  return [
    ...
    '- When `review/telemetry.json` reports `status: succeeded` with `review_outcome: bounded-success` (or a legacy succeeded payload with a preserved `termination_boundary`), record that wrapper execution completed with a successful bounded review outcome, not as a wrapper blocker or generic quiet-tail failure; this does not override `review_verdict`, and clean handoff still requires `review_verdict: clean`.',
