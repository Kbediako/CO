...
export function buildProviderWorkerPrompt(
  ...
  if (turnNumber > 1) {
    return [
      ...
      `- For app-touching lanes, inspect permit posture with \`${helperCommand} runtime-proof --issue-id ${issue.id} --origin <app-url> --format json\`, then generate reviewer-usable handoff content with \`${helperCommand} runtime-proof --issue-id ${issue.id} --origin <app-url> --kind <screenshot|external-link|video> --proof-url <reviewer-url> --title <label> --summary <what changed> --format json\`; the helper fails closed when the permit blocks the mode, when the proof URL is loopback/local-only, or when only a local file path is available.`,
