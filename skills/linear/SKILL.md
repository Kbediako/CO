...
- In provider-worker issue workspaces, audited `linear child-stream` and `linear child-lane` runs record manifests under the workspace-scoped artifact root for that issue workspace. Treat those manifests as the intended delegation evidence path; do not reach for blanket `DELEGATION_GUARD_OVERRIDE_REASON` text when valid child evidence already exists.
