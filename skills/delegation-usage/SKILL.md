...
- App-server model/list evidence under ChatGPT auth shows `gpt-5.5` with `xhigh` available; `gpt-5.4` may still appear as the app-server `isDefault`.
...
- Recommended baseline in `~/.codex/config.toml`:
  ...
  - Optional `[agents.awaiter]` override -> `~/.codex/agents/awaiter-high.toml` when you want awaiter at `gpt-5.5` + `xhigh` while preserving awaiter instructions
  - `[agents.worker_complex]` -> `~/.codex/agents/worker-complex.toml` (`gpt-5.5`, `xhigh`)
- Keep delegated subagent and review surfaces on the CO-local `gpt-5.5` posture under ChatGPT auth.
- Caveat: app-server `isDefault` may still report `gpt-5.4`.
