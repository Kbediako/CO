...
- `NOTES="Goal: ... | Summary: ... | Risks: ..." codex-orchestrator review --task <task-id>` — run standalone review wrapper with manifest-backed evidence (supports run-review flags/env); in non-interactive/CI runs it prints the handoff prompt and exits unless `FORCE_CODEX_REVIEW=1`.
  - Default bounded review mode keeps heavy commands advisory, with a low-signal nearby-inspection drift guard plus a bounded meta-surface expansion guard on that non-heavy path; tune or disable them with `CODEX_REVIEW_LOW_SIGNAL_TIMEOUT_SECONDS=<seconds>` and `CODEX_REVIEW_META_SURFACE_TIMEOUT_SECONDS=<seconds>` (`0` disables, default `180` for each).
