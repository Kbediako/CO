...
- Current CO-local Codex CLI `0.135.0` ChatGPT-auth/appserver posture
...
The npm CLI install is the baseline. Codex plugin marketplace setup is additive for Codex releases that expose plugin flows. Current local Codex CLI `0.135.0` keeps marketplace management under `codex plugin marketplace ...`, while release-facing smoke workflow pins intentionally hold on the version policy target:
