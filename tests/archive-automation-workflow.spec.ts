...
describe('archive automation workflow required checks', () => {
  it('exposes Core Lane as a dispatchable required-check workflow for archive PR heads', async () => {
    ...
    expect(diffBudgetStep.if).toContain('inputs.archive_pr_number');
    ...
    expect(overrideStep.if).toContain('inputs.archive_pr_number');
  ...
  it('dispatches Core Lane after creating or updating an archive PR', async () => {
    ...
    expect(dispatchStep.run).toContain('create-pull-request did not expose the archive PR branch/head SHA');
  });
