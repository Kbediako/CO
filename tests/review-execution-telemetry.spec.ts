...
describe('review-execution-telemetry', () => {
  ...
  it('does not treat an inspected review-log slice starting at codex as the final verdict', async () => {
    const sandbox = await makeSandbox();
    const outputLogPath = await writeReviewOutput(
      sandbox,
      [
        ...
        ' succeeded in 0ms:',
        'codex',
        '- [P2] Tail-sliced stale finding should not become current verdict',
        ''
      ].join('\n')
    );
    const telemetryPath = join(sandbox, 'review', 'telemetry.json');
