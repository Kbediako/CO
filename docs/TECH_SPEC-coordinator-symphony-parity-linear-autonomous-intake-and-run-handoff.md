...
- Live follow-up `1305` narrows the remaining contract drift for that fallback path: strict `delegation-guard` currently rejects the provider-started fallback task ids before downstream work starts, so guard alignment remains a separate bounded fix rather than a reason to reopen the broader 1303 lane.
