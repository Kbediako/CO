...
## Summary
- Objective: preserve the CO-486 decision that persisted Codex goal state is advisory provider-worker run evidence.
- Scope: live Codex CLI `0.128.0` goal/app-server/tool probes, advisory evidence contract design, provider-worker/long-poll/hook recovery classification, docs packet, registry mirrors, Rework provenance, and follow-up scope decision.
- Constraints: no hook/resume integration in this lane, no Linear authority replacement, no release-facing Codex pin promotion, no broad provider-worker rewrite, and no parent-owned lifecycle mutation by this child lane.

## Rework Provenance
- Prior PR `#751` was closed after the Rework reset and is not the active PR lifecycle surface.
- Parent registry reset commit `e8a904d8c089` restored the CO-486 registry/task snapshot on the current main-based branch.
- Stale appserver child lane `docs-packet-rebase` was invalidated and must not be treated as the packet producer.
- Current child packet producer is `docs-packet-rebase-r2`, anchored at `ctx:sha256:968e79825ece74f1535532921c7bb2fe3b53e6ab34bad59ecefe3796d897cfe3#chunk:c000001`.
- Current r2 manifest path: `.runs/linear-6a92b5d9-3293-4e27-9bc5-28c8c62becfc-docs-packet-rebase-r2/cli/2026-05-02T19-51-38-188Z-83e5dfff/manifest.json`.
- Parent owns Linear state, workpad state, registries, docs-review, implementation, validation, PR lifecycle, and final handoff. This child lane owns only the declared packet files.

## Issue-Shaping Contract
- User-request translation carried forward: CO-486 should preserve live Codex CLI `0.128.0` persisted `/goal` evidence and define the evidence contract before implementation.
...
- Nearby wrong interpretations to reject: goal state replaces Linear/workpad state; hook/resume integration is in scope now; this is a generic CO-466 release duplicate; goal completion alone means the issue can transition to review or done; PR `#751` remains active; invalidated `docs-packet-rebase` is authoritative.
- Explicit non-goals carried forward: no release pin promotion, no hook/resume integration, no broad provider-worker rewrite, no replacement of Linear as workflow authority, and no parent-owned lifecycle mutation by this child lane.
...
- Explicitly out-of-scope differences: runtime hook/resume integration, TUI automation, cloud/release pin movement, provider-worker authority refactors, and this child lane changing registries or lifecycle state.

## Readiness Gate
- Not done if:
  - live `0.128.0` goal feature/app-server/tool probes are missing from the packet
  - evidence fields are not specified
  - provider-worker, long-poll, hook recovery, runtime continuation, TUI, child-lane, and operator-continuity classifications are missing
  - follow-up implementation scope is not classified
  - Linear/workpad authority can be bypassed by goal state
  - stale PR `#751` or invalidated `docs-packet-rebase` is treated as active authority
- Pre-implementation issue-quality review evidence:
  ...
  - 2026-05-02T19:51:38Z: Rework packet recreation is docs-only and must preserve parent Linear/workpad/PR authority.
- Safeguard ownership split:
  - Parent owns Linear truth, workpad, registries, canary proof acceptance, docs-review, validation, follow-up lifecycle, PR lifecycle, and final handoff.
  - This r2 child lane owns only the six declared packet files until parent acceptance.
...
- Functional requirements:
  ...
  3. Preserve model-tool goal state evidence from the provider-worker session.
  4. Preserve runtime continuation surface evidence.
  ...
  7. Preserve the bounded implementation follow-up decision without implementing hook/resume behavior in this lane.
- Non-functional requirements:
  - evidence must remain reproducible from local commands or tool outputs
  - contract must fail closed when goal evidence is missing, stale, disabled, unavailable, paused, budget-limited, complete, or thread-mismatched
  - no lifecycle gate may trust goal state ahead of Linear/workpad/PR/review/check truth
  - workpad rendering should remain compact and pointer-oriented
- Interfaces / contracts:
  ...
  - model tools observed in the worker session: `get_goal`, `create_goal`, and `update_goal`
...
- Direct model-tool canary in the provider-worker session:
...
- Rationale:
  ...
  - goal state can be missing, stale, disabled, unavailable, paused, budget-limited, complete, or thread-mismatched without implying issue state
...
## Validation Plan
- Child-lane checks:
  - source payload availability check for the r2 source anchor
  - scoped changed-file review confirming only declared packet files changed
  - scoped trailing-whitespace hygiene over declared packet files
  - protected-term scan for CO-486 terms and Rework authority boundaries
- Parent-owned checks:
  ...
  - final standalone review/elegance and PR lifecycle gates as parent-owned work
- Rollout verification: workpad records the canary decision, follow-up decision, Rework truth, validation state, and review/elegance result before review handoff.
...
## Approvals
- Prior docs-review: `.runs/linear-6a92b5d9-3293-4e27-9bc5-28c8c62becfc-docs-review/cli/2026-05-02T15-25-40-851Z-0acbfb39/manifest.json`; P2 persistence concern addressed with exec/resume evidence.
...
- Final standalone/elegance review: parent-owned; provider-worker handoff evidence must be recorded in the Linear workpad and PR lifecycle rather than self-authorized in this tracked packet.
- Date: 2026-05-02.
