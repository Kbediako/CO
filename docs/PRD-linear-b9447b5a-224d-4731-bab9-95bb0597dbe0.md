...
- Use source-specific lifecycle evidence for the May 19 and May 20 stale historical task/report cohorts and direct review evidence for strict pre-expiry public/skill/template/spec entries.
...
| Strict pre-expiry surfaces | Skill/template and active spec rows are inside the pre-expiry window. | Current guidance/specs require direct review before handoff. | Registry `last_review` changes are backed by explicit CO-558 review evidence, while legacy spec source files stay unchanged when no content updates are needed. | Metadata-only bumps without reviewing source content. |
...
- Account for May 19 and May 20 `.agent/task`, task packet, report-only, public/skill/template, and spec pre-expiry rows.
...
- Review date: 2026-05-20 for the current Apr 19/public/spec refresh; May 19 remains the Apr 18 owner re-home evidence date.
