...
- User intent / needs:
  ...
  - handle the CO-520-style stale blocker snapshot path by letting direct issue-by-id proof for the released claim's own Backlog/backlog state create and reuse `passive_release`, even when another issue still carries stale Blocked/started blocker metadata for that claim
...
- A CO-520-style stale blocker snapshot is covered by a regression where direct issue-by-id verifies Backlog/backlog, records `passive_release`, skips repeated direct reads, and does not start/resume work.
...
| Passive Backlog released not_active classification | Parked Backlog released claims can still fall through to direct issue-by-id and look like active stuck refresh work. | remove fallback | CO-571 | CO-529-style released `provider_issue_released:not_active` Backlog/backlog row with no active run, retry, promotion, merge-closeout, or cancelable retained-run evidence, including CO-521/CO-524 no-current-poll verification and CO-520 stale blocker-snapshot recurrence. | Observed 2026-05-21 | 2026-05-21 | This issue | Passive Backlog/not_active rows skip repeated direct issue-by-id only after recent explicit `passive_release` verification for the claim's own Backlog snapshot, while active/reopened/pending-review/retry/Blocked/merge-closeout paths still revalidate. | Focused CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification, CO-520 stale blocker-snapshot regression, and active direct issue-by-id regressions. |
