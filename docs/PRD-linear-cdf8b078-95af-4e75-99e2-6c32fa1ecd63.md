...
- User intent / needs:
  ...
  - handle the CO-521/CO-524-style Backlog/backlog released `not_active` path when the current poll snapshot is unavailable and the row has no retained run, retry, review-promotion, or merge-closeout metadata
...
| Passive Backlog released not_active | After PR #858 merged, CO-529 was Backlog/backlog, released as `provider_issue_released:not_active`, and still drove `claim_issue_by_id:released` when the current source had WIP `0/3`. After PR #860 merged, CO-521 and CO-524 reproduced the same loop when the current poll snapshot was unavailable. | Backlog is parked, not a direct issue-by-id recovery trigger; promotion/reopen evidence should arrive through current poll/fresh discovery/autopilot, while Blocked/retry/pending-review/merge-closeout paths keep their existing revalidation. A complete inert cached Backlog row does not need a current poll map to stay passive. | Released Backlog/not_active rows with no active run, no retry fields, no promotion/merge-closeout metadata, and no cancelable retained run skip direct issue-by-id and do not set `restart_required`, including no-current-poll-snapshot refreshes. | Blanket-suppressing released claims, hiding Blocked work, or blocking active/reopened/pending-review/retry/merge-closeout evidence. |
...
- A passive Backlog/backlog released `not_active` row with no active/retry/promotion/merge-closeout evidence can still enter `claim_issue_by_id:released` only because the current poll snapshot is skipped or unavailable.
