...
- The first eighth-rework review found the direct-read budget could still starve tail rows if time alone expired earlier passive proofs. The final design treats exact-snapshot direct issue-by-id `passive_release` proof as valid until the Linear issue snapshot changes, so unchanged early rows cannot repeatedly consume the front of the budget.
...
- Stop parked Backlog released `not_active` rows before repeated direct issue-by-id only when there is no active run, retry, promotion, merge-closeout, or cancelable retained-run and a matching explicit `passive_release` direct issue-by-id verification matches the same Backlog snapshot.
...
| Passive Backlog released not_active classification | Parked Backlog released claims can still fall through to direct issue-by-id and look like active stuck refresh work. | remove fallback | CO-571 | CO-529-style released `provider_issue_released:not_active` Backlog/backlog row with no active run, retry, promotion, merge-closeout, or cancelable retained-run evidence, including CO-521/CO-524 no-current-poll verification, CO-520 stale blocker-snapshot recurrence, and large no-poll Backlog cohorts. | Observed 2026-05-21 | 2026-05-21 | This issue | Passive Backlog/not_active rows skip repeated direct issue-by-id only after matching explicit `passive_release` verification for the claim's own Backlog snapshot; matching direct proof wins over stale blocker metadata, unverified no-poll rows use a per-refresh direct-read budget with deferred progress, and active/reopened/pending-review/retry/Blocked/merge-closeout paths still revalidate or start work. | Focused CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification, CO-520 stale blocker-snapshot regression, large no-poll Backlog cohort budgeting, and active direct issue-by-id regressions. |
