...
- PR #860 fixed stale retained merged-closeout history, but live latest-main proof after merge `65b05d2525a6c0fe793bf778d6f42147bc75aceb` still reproduced `refresh:claim_issue_by_id_reconcile` / `claim_issue_by_id:released` for CO-521 / `linear:9749edb3-51e3-45e3-935b-8333dafafca5` and then CO-524 / `linear:0d975e30-67a5-4ea4-992a-61576c89d913`. Both are released `provider_issue_released:not_active` Backlog/backlog claims with no retained run, retry, review-promotion, or merge-closeout metadata when the current poll snapshot is unavailable. This rework must apply the parked Backlog passive classifier only after direct issue-by-id verifies the same inert Backlog snapshot and records explicit `passive_release` metadata.
- PR #861 fixed no-current-poll Backlog verification, but live latest-main proof after merge `c66abd777d74aebbf9f8b046f54581962ad7703a` still reproduced `refresh:claim_issue_by_id_reconcile` / `claim_issue_by_id:released` for CO-520 / `linear:617541e3-39f0-4926-8105-9b2a5c053a5b`. Direct issue-by-id confirmed the released claim's own issue was Backlog/backlog, but a stale blocker snapshot still marked it Blocked/started and suppressed the `passive_release` marker. This rework must make direct own-issue Backlog verification outrank stale blocker snapshots for passive classification, without hiding active direct truth.
- PR #862 fixed stale blocker marker creation, but live current-source proof after merge `afc33fe2ebbb24b62f44dc7fc7811ad0aba28029` still reproduced released Backlog no-poll reconciliation loops for CO-521 and CO-524 after the control host restarted onto current source. This rework must make matching direct own-issue `passive_release` proof suppress repeated direct reads even when stale blocker metadata persists, and it must cap unverified no-poll Backlog direct reads per refresh so large cohorts make deferred progress instead of tripping the lifecycle watchdog.
- The first eighth-rework review found the direct-read budget could still starve tail rows if time alone expired earlier passive proofs; PR #863 Codex review then found exact-snapshot proof could not be permanent when polling stays unavailable and the issue later becomes active. The final design treats exact-snapshot direct issue-by-id `passive_release` proof as valid for immediate repeated reads, prioritizes unverified tail rows, and revalidates stale exact-snapshot proof through the same bounded budget after unverified rows have priority.
- Desired Outcome: terminal released historical claims, parked Backlog released not-active claims, stale retained merged-closeout history, and stale blocker snapshots remain truthful audit evidence, but they do not manufacture a control-host restart loop when current authority shows no active work. Real active refresh stalls must still fail closed with debuggable `provider_refresh_lifecycle_stuck` and `restart_required` evidence.
...
- User intent / needs:
  ...
  - handle the CO-520-style stale blocker snapshot path by letting direct issue-by-id proof for the released claim's own Backlog/backlog state create `passive_release`, even when another issue still carries stale Blocked/started blocker metadata for that claim
  - handle large no-current-poll Backlog released cohorts by bounding direct issue-by-id proof reads per refresh, recording `claim_issue_by_id:released_deferred` for deferred rows, and letting deferred rows receive direct verification in later refreshes
...
- Protected terms / exact artifact and surface names:
  ...
  - PR #862
  - `afc33fe2ebbb24b62f44dc7fc7811ad0aba28029`
  - stale blocker snapshot
  - direct own-issue Backlog verification
  - `passive_release`
  - `claim_issue_by_id:released_deferred`
...
- Nearby wrong interpretations to reject:
  ...
  - letting stale blocker snapshots outrank matching direct issue-by-id proof for the released claim's own issue state
  - letting large no-poll Backlog released cohorts exhaust the refresh lifecycle before deferred rows can receive direct proof
...
| Passive Backlog released not_active | After PR #858 merged, CO-529 was Backlog/backlog, released as `provider_issue_released:not_active`, and still drove `claim_issue_by_id:released` when the current source had WIP `0/3`. After PR #860 merged, CO-521 and CO-524 reproduced the same loop when the current poll snapshot was unavailable. After PR #861 merged, CO-520 reproduced the loop because a stale blocker snapshot still said Blocked/started even though direct issue-by-id verified the claim's own issue was Backlog/backlog. After PR #862 merged, current-source control-host refresh still looped through released Backlog no-poll rows because matching passive proof did not suppress repeated reads when stale blocker metadata persisted and large cohorts were not budgeted. | Backlog is parked, not a repeated direct issue-by-id recovery trigger; promotion/reopen evidence should arrive through current poll/fresh discovery/autopilot, while Blocked/retry/pending-review/merge-closeout paths keep their existing revalidation. When no current poll snapshot exists or stale blocker metadata disagrees, the passive decision must bind to explicit direct issue-by-id truth for the released claim's own Backlog snapshot, and stale exact-snapshot proof must later revalidate if polling remains unavailable. | Released Backlog/not_active rows with no active run, no retry fields, no promotion/merge-closeout metadata, and no cancelable retained run skip repeated direct issue-by-id only after a matching explicit `passive_release` direct issue-by-id verification for the same issue state/update snapshot; matching direct proof wins over stale blocker metadata, unverified no-poll rows consume a per-refresh direct-read budget, deferred rows are marked as `claim_issue_by_id:released_deferred`, stale exact-snapshot proofs re-enter the budget after unverified rows have priority, and active or reopened direct truth still starts work. | Blanket-suppressing released claims, hiding Blocked work, blocking active/reopened/pending-review/retry/merge-closeout evidence, permanently trusting stale proof while polling is unavailable, or deleting deferred claim evidence. |
...
- A passive Backlog/backlog released `not_active` row with no active/retry/promotion/merge-closeout evidence can be hidden during a no-current-poll refresh without a matching explicit `passive_release` direct issue-by-id verification for the same Backlog snapshot.
...
- A large no-poll Backlog released cohort can exhaust the refresh lifecycle instead of using a bounded direct-read budget and deferred progress.
- Stale exact-snapshot `passive_release` proof can permanently suppress direct revalidation while polling remains unavailable and the issue later becomes active.
- Deferred `claim_issue_by_id:released_deferred` rows never progress to direct verification in later refreshes, including when earlier direct proofs are older than the prior time-based proof window but still match the same Linear issue snapshot.
...
- Stop parked Backlog released `not_active` rows before repeated direct issue-by-id only when there is no active run, retry, promotion, merge-closeout, or cancelable retained-run and a matching explicit `passive_release` direct issue-by-id verification matches the same Backlog snapshot; stale exact-snapshot proof revalidates on the bounded budget when polling remains unavailable.
- Stop stale blocker snapshots from vetoing matching direct own-issue Backlog verification for parked released `not_active` rows.
- Bound unverified no-poll Backlog direct proof reads per refresh and carry deferred rows forward until later verification.
...
- A CO-520-style stale blocker snapshot is covered by a regression where direct issue-by-id verifies Backlog/backlog, records `passive_release`, matching direct proof suppresses repeated reads despite stale blocker metadata, and no work starts.
- A large no-poll Backlog released cohort is covered by a regression where direct issue-by-id reads are capped per refresh, `claim_issue_by_id:released_deferred` rows remain visible, deferred rows progress over later refreshes even after time advances beyond the previous passive-proof window, and verified passive rows do not start/resume work.
- Stale exact-snapshot `passive_release` proof is covered by a regression where polling remains unavailable, direct issue-by-id later reports active truth, and the worker starts instead of trusting the stale proof permanently.
...
- Focused Backlog released `not_active` coverage for the CO-520 stale blocker-snapshot recurrence.
- Focused Backlog released `not_active` coverage for large no-poll direct-read budgeting and deferred progress.
...
| Passive Backlog released not_active classification | Parked Backlog released claims can still fall through to direct issue-by-id and look like active stuck refresh work. | remove fallback | CO-571 | CO-529-style released `provider_issue_released:not_active` Backlog/backlog row with no active run, retry, promotion, merge-closeout, or cancelable retained-run evidence, including CO-521/CO-524 no-current-poll verification, CO-520 stale blocker-snapshot recurrence, large no-poll Backlog cohorts, and stale exact-snapshot proof while polling remains unavailable. | Observed 2026-05-21 | 2026-05-21 | This issue | Passive Backlog/not_active rows skip repeated direct issue-by-id only after matching explicit `passive_release` verification for the claim's own Backlog snapshot; matching direct proof wins over stale blocker metadata, unverified no-poll rows use a per-refresh direct-read budget with deferred progress, stale exact-snapshot proof revalidates after unverified rows have priority, and active/reopened/pending-review/retry/Blocked/merge-closeout paths still revalidate or start work. | Focused CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification, CO-520 stale blocker-snapshot regression, large no-poll Backlog cohort budgeting, stale exact-snapshot proof revalidation, and active direct issue-by-id regressions. |
...
- Steady-state proof: raw released claim rows remain source-labeled audit evidence, while terminal released `not_active` claims, passive Backlog released `not_active` claims with matching `passive_release` direct issue-by-id verification for the same issue snapshot, and stale retained merged-closeout rows with complete cached metadata, null retry fields, and no active or cancelable retained run do not drive `restart_required` or retrying WIP. Unverified no-poll Backlog rows are direct-read within a per-refresh budget, deferred rows remain visible until subsequent refreshes verify them, and stale exact-snapshot proofs revalidate through the same budget after unverified rows have priority.
- Tests/docs: `ProviderIssueHandoff.test.ts` terminal released metadata-only table, no-current-poll-snapshot regression, current-poll terminal snapshot regression, map-missing terminal snapshot regression, CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification and active direct issue-by-id regressions, CO-520 stale blocker-snapshot regression, large no-poll Backlog cohort budgeting regression, stale exact-snapshot proof active revalidation regression, CO-522 stale retained merged-closeout regressions, stale/current `review_promotion` regressions, active-stuck regression, `ControlRuntime.test.ts` retry projection regression, and this CO-571 packet.
