...
- User intent / needs:
  ...
  - handle the CO-520-style stale blocker snapshot path by letting direct issue-by-id proof for the released claim's own Backlog/backlog state create `passive_release`, even when another issue still carries stale Blocked/started blocker metadata for that claim, while continuing bounded revalidation as long as blocker-mismatched poll evidence persists
...
- Nearby wrong interpretations to reject:
  ...
  - letting stale blocker snapshots outrank direct issue-by-id proof for the released claim's own issue state
...
| Passive Backlog released not_active | After PR #858 merged, CO-529 was Backlog/backlog, released as `provider_issue_released:not_active`, and still drove `claim_issue_by_id:released` when the current source had WIP `0/3`. After PR #860 merged, CO-521 and CO-524 reproduced the same loop when the current poll snapshot was unavailable. After PR #861 merged, CO-520 reproduced the loop because a stale blocker snapshot still said Blocked/started even though direct issue-by-id verified the claim's own issue was Backlog/backlog. | Backlog is parked, not a repeated direct issue-by-id recovery trigger; promotion/reopen evidence should arrive through current poll/fresh discovery/autopilot, while Blocked/retry/pending-review/merge-closeout paths keep their existing revalidation. When no current poll snapshot exists or a blocker snapshot disagrees, the passive decision must bind to explicit direct issue-by-id truth for the released claim's own Backlog snapshot. | Released Backlog/not_active rows with no active run, no retry fields, no promotion/merge-closeout metadata, and no cancelable retained run skip repeated direct issue-by-id only after a recent explicit `passive_release` direct issue-by-id verification for the same issue state/update snapshot and no blocker-mismatched poll evidence; stale blocker snapshots cannot veto creation of that marker, but blocker mismatches keep forcing bounded revalidation, and active or reopened direct truth still starts work. | Blanket-suppressing released claims, hiding Blocked work, or blocking active/reopened/pending-review/retry/merge-closeout evidence. |
...
- Stop parked Backlog released `not_active` rows before repeated direct issue-by-id only when there is no active run, retry, promotion, merge-closeout, cancelable retained-run, or blocker-mismatched poll evidence and a recent explicit `passive_release` direct issue-by-id verification matches the same Backlog snapshot.
- Stop stale blocker snapshots from vetoing recent direct own-issue Backlog verification for parked released `not_active` rows.
...
- A CO-520-style stale blocker snapshot is covered by a regression where direct issue-by-id verifies Backlog/backlog, records `passive_release`, continues bounded revalidation while blocker-mismatched poll evidence persists, and does not start/resume work.
...
| Passive Backlog released not_active classification | Parked Backlog released claims can still fall through to direct issue-by-id and look like active stuck refresh work. | remove fallback | CO-571 | CO-529-style released `provider_issue_released:not_active` Backlog/backlog row with no active run, retry, promotion, merge-closeout, or cancelable retained-run evidence, including CO-521/CO-524 no-current-poll verification and CO-520 stale blocker-snapshot recurrence. | Observed 2026-05-21 | 2026-05-21 | This issue | Passive Backlog/not_active rows skip repeated direct issue-by-id only after recent explicit `passive_release` verification for the claim's own Backlog snapshot and no blocker-mismatched poll evidence, while active/reopened/pending-review/retry/Blocked/merge-closeout paths and blocker mismatches still revalidate. | Focused CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification, CO-520 stale blocker-snapshot regression, and active direct issue-by-id regressions. |
...
- Steady-state proof: raw released claim rows remain source-labeled audit evidence, while terminal released `not_active` claims, passive Backlog released `not_active` claims with recent matching `passive_release` direct issue-by-id verification and no blocker-mismatched poll evidence, and stale retained merged-closeout rows with complete cached metadata, null retry fields, and no active or cancelable retained run do not drive `restart_required` or retrying WIP.
