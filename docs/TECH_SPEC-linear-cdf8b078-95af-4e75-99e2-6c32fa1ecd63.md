...
## Validation
Implementation must add focused coverage for both released terminal claim paths, the Duplicate/canceled terminal example, the selected-claim retry projection mismatch, the no-current-poll-snapshot path before direct issue-by-id, and stale/current `review_promotion` classification. It must also keep a real active/stuck path failing closed. Required gates include spec guard, build, lint, test, docs checks, freshness, diff budget, pack smoke for downstream control-host behavior, and review/elegance evidence where feasible.
