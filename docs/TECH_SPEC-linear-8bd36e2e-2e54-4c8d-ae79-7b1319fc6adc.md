...
- For retained contracts: strict provenance and task-registration checks are supported correctness gates, not temporary fallback behavior.
- Large-refactor check: no large refactor is required if parent can isolate the fix to child-stream task id construction and guard-side provider contract recognition. Relaunch only if the parent proves task identity cannot be made consistent without a broader provider-worker launch redesign.
...
- Tests / checks:
  ...
  - parent-selected broader build/lint/test/docs/review gates before handoff
