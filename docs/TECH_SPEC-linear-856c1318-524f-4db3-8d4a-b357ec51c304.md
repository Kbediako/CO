...
- Scope:
  ...
  - one bounded archive matcher plus task-index discovery fix in `scripts/implementation-docs-archive.mjs`
...
- Functional requirements:
  ...
  - task-linked doc extraction must include linked PRD/TECH_SPEC/ACTION_PLAN docs when they match policy patterns, whether the task index uses legacy `path` or canonical `paths.docs`
...
- Non-functional requirements (performance, reliability, security):
  ...
  - avoid changes to secrets, auth, or external workflow credentials
- Interfaces / contracts:
  - input contract: `docs/implementation-docs-archive-policy.json` `doc_patterns`
  - implementation contract: `globToRegExp(...)` plus canonical `paths.docs` discovery in `scripts/implementation-docs-archive.mjs`
  - regression contract: `tests/implementation-docs-archive.spec.ts` must prove linked docs matching plain `*` patterns archive into payloads and stubs for both legacy `path` and canonical `paths.docs`

## Architecture & Data
- Architecture / design adjustments:
  - no architecture change; this is a policy-to-implementation alignment fix within the existing archiver
  - canonical `tasks/index.json` entries seed archive discovery through `paths.docs`, so the archiver must read both that field and legacy `path`
...
- Tests / checks:
  ...
  - focused archive regression coverage
