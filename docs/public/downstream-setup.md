...
- Marketplace packaging is shipped as an additive registration path for newer Codex releases that expose `codex plugin marketplace add`. npm remains the supported baseline because it is the simplest supported CLI install path.
...
- After uninstalling the plugin from `/plugins` or disabling it in `${CODEX_HOME:-~/.codex}/config.toml`, run `codex plugin marketplace remove codex-orchestrator` if you no longer want Codex to read the shipped marketplace registration. If you later want to re-enable the installed plugin, or restore that marketplace source, re-run `codex plugin marketplace add ...` first.
