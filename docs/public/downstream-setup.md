...
- CO-local ChatGPT-auth/appserver posture now uses `gpt-5.5` / `xhigh` on Codex CLI `0.135.0` when live access smoke passes; release-facing cloud/downstream pins remain evidence-gated in the version policy.
...
- CO-196 posture lineage remains unchanged: npm is the supported baseline because it is the simplest supported CLI install path, and marketplace packaging is an additive registration path for newer Codex releases. `0.121.0` accepts both `codex marketplace add` and `codex plugin marketplace add`; `0.122.0+` require `codex plugin marketplace add`, and current policy evidence also exposes `codex plugin marketplace upgrade` / `remove`.
