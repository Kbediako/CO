...
- Acceptance criteria: `/ui/machine-status.json` reads only committed immutable snapshots; freshness/status collection moves outside the serving event loop with bounded async operations, cancellation, no same-root overlap, and stale-on-failure semantics; provider-intake admission requires collector-verified source-root authority bound to owner/run/source-root snapshot with explicit observed/expiry metadata and never treats provider polling success as freshness evidence; `/healthz` is control-token-authenticated cheap liveness, `/readyz` is degraded readiness; current freshness/owner generation is monotonic across `co-status`, `live_host`, supervision, and UI status; clean shared root plus dirty isolated workspace stays clean; same-endpoint diagnostics point to endpoint starvation or snapshot-read blocking.
...
5. Add explicit source-root freshness authority metadata from the collector, preserve that authority across later provider refresh starts unless a collector replaces the owner snapshot, and fail provider-intake closed when verification is missing, malformed, stale, expired, missing the matching freshness observation, missing the freshness snapshot source-root realpath, carried from a previous owner after startup live-owner replacement, or not bound to the same owner token, run id, and source-root realpath.
