...
| Child-lane bounds | Runtime supports launch, accept, reject, and invalidate. | Parallel-first still needs bounded resource use. | A cap counts active, pending, and unaccepted child lanes; stale in-flight accept claims older than 30 minutes and legacy claims without timestamps are recoverable; cap exhaustion is explicit and cannot bypass `CO-125`. |
