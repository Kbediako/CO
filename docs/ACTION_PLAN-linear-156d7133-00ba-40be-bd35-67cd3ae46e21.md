...
## Summary
- Goal: finish `CO-25` by adding an explicit, safe shared-root reconciliation step to autonomous merged closeout so the same workpad says whether `/Users/kbediako/Code/CO` was aligned or intentionally skipped.
- Scope: docs-first packet, pre-implementation docs-review, provider-worker prompt updates, repo-local `linear` / `land` skill updates, focused prompt-contract tests, required validation, and workpad refresh.
- Assumptions:
  - the missing seam is the provider-worker closeout contract, not a deeper control-host workspace-management gap
...
3) Update `orchestrator/src/cli/providerLinearWorkerRunner.ts` so first-turn and continuation prompts require shared-root inspection, safe fast-forward-only sync, and explicit skip-reason evidence before `Done`.
4) Update `skills/linear/SKILL.md` and `skills/land/SKILL.md` so merged closeout guidance matches the new shared-root reconciliation contract.
5) Update focused prompt/contract tests in `orchestrator/tests/ProviderLinearWorkerRunner.test.ts`.
6) Run the required validation floor, refresh the workpad with current evidence, and prepare the branch for PR/review handoff.
