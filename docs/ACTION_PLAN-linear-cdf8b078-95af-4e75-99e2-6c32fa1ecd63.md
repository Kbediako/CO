...
- Not done if:
  ...
  - passive Backlog/backlog released `not_active` still records `claim_issue_by_id:released` only because the current poll snapshot is skipped or unavailable
...
- Pre-implementation issue-quality review:
  ...
  - 2026-05-21 sixth rework: after PR #860 merged, live latest-main evidence at `65b05d2525a6c0fe793bf778d6f42147bc75aceb` still hit `claim_issue_by_id:released` for CO-521 and CO-524. The rework must treat passive Backlog released `not_active` rows as passive even when the current poll snapshot is unavailable, while preserving active/reopened/pending-review/retry/Blocked/merge-closeout revalidation.
...
- Checks / tests:
  ...
  - focused CO-521 Backlog released `not_active` no-current-poll direct issue-by-id regression
