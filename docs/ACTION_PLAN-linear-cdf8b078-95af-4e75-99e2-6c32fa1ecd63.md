...
- Assumptions:
  ...
  - After initial PR #861 review, Codex found that hiding no-current-poll Backlog claims without direct issue-by-id verification could hide active direct truth; the final fix must record explicit `passive_release` metadata for the same Backlog snapshot before skipping repeated direct reads.
  ...
  - After PR #862 merged, current-source control-host proof showed the remaining passive-proof budget path: matching direct own-issue `passive_release` proof still lost to stale blocker metadata for repeated-read suppression, and large no-poll Backlog cohorts could exhaust refresh lifecycle before every parked row received direct proof.
  - After the first eighth-rework review, the final budget design must avoid time-only passive-proof expiry that rereads early rows and starves deferred tail rows.
...
- Not done if:
  ...
  - passive Backlog/backlog released `not_active` is hidden during a no-current-poll refresh without a matching explicit `passive_release` direct issue-by-id verification for the same Backlog snapshot
...
- Pre-implementation issue-quality review:
  ...
  - 2026-05-21 eighth rework: after PR #862 merged, live current-source evidence at `afc33fe2ebbb24b62f44dc7fc7811ad0aba28029` still looped through released Backlog no-poll claim reconciliation for CO-521 and then CO-524. Matching exact-snapshot direct own-issue `passive_release` must suppress repeated reads even when stale blocker metadata persists, unverified large no-poll Backlog cohorts must use bounded per-refresh direct proof with `claim_issue_by_id:released_deferred` progress that cannot be starved by time-only expiry of earlier exact-snapshot proofs, and stale exact-snapshot proof must revalidate after unverified rows have priority.
...
- Checks / tests:
  ...
  - focused large no-poll Backlog released `not_active` direct-read budget and deferred-progress regression
