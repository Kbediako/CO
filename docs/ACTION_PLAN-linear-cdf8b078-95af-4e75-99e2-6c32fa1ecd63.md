...
4. Add focused failing regressions for CO-472 / CO-469 `claim_issue_by_id:released`, CO-461 / CO-471 `claim_reconcile:released`, selected-claim retry projection, no-current-poll-snapshot direct issue-by-id suppression, accepted pending-revalidation rechecks, stale/current `review_promotion` including deferred-poll fail-closed, and a real active stuck path.
