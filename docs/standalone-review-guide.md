...
- In `audit` mode, the wrapper keeps the meta-surface expansion guard active for unrelated drift but treats the explicitly allowed audit evidence surface (`run-manifest` today) as in-scope.
