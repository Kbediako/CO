...
- Explicit wrapper scope flags (`--uncommitted`, `--base`, `--commit`) keep the full saved prompt/context in `review/prompt.txt`, launch `codex review` without any prompt argument because current Codex CLI still treats stdin (`-`) as `[PROMPT]` and rejects it with scope flags, and use bounded `--title` transport for reviewer-visible scoped context. Unscoped wrapper runs still pass the saved prompt/context inline.
