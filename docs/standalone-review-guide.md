...
- Use `--surface audit` (or `CODEX_REVIEW_SURFACE=audit`) when you explicitly want checklist/manifest/PRD/evidence validation in the prompt.
...
- In that same default `diff` path, a meta-surface expansion guard can fail closed when the review persistently broadens into off-task review-orchestration surfaces such as global skills/memory or run manifests/logs; disable it with `CODEX_REVIEW_META_SURFACE_TIMEOUT_SECONDS=0` or shorten/extend it with an explicit seconds value.
- In `audit` mode, those audit surfaces are in-scope, so the wrapper disables the meta-surface expansion guard for that run while keeping the other runtime guards active.
