...
- `codex-orchestrator review`: writes task-scoped manifest evidence into the saved review prompt artifact, applies diff-budget checks when `scripts/diff-budget.mjs` exists in the target repo root (auto-skips in downstream npm installs that do not ship that helper), passes that prompt inline for unscoped launches, omits it entirely for explicit scoped launches, and may emit a handoff prompt in non-interactive mode (see above).
