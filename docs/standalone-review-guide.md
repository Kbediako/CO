...
- In that default bounded path, an immediate command-intent boundary now fails closed on explicit package-manager validation suites (`npm` / `pnpm` / `yarn` / `bun` `run test|lint|build|docs:check|docs:freshness`), direct `vitest`/`jest`-style launches, nested `codex review` / `codex-orchestrator review` / `codex-orchestrator start docs-review|implementation-gate|diagnostics`, and mutating `delegation.delegate.spawn|pause|cancel` tool calls; help-only lookups such as `codex review --help` remain read-only inspection, and `delegation.delegate.status` is not an immediate boundary violation, but repeated status-only meta inspection can still contribute to the sustained meta-surface guard. For explicit scoped bounded runs, a command-intent boundary triggers exactly one stricter retry before the wrapper reports failure; that retry moves the original `--uncommitted`, `--base`, or `--commit` scope into a reviewer-visible inline prompt, names the blocked attempt as follow-up-only, and adds `default_permissions=":read-only"` as the 0.128 read-only permission-profile override. If the active Codex CLI rejects `default_permissions`, the wrapper may attempt exactly one extra legacy `sandbox_mode="read-only"` compatibility retry before reporting failure. A successful retry records `review_outcome: bounded-success` with the preserved `termination_boundary.kind: command-intent` and carries the original command-intent aggregate counts into the success telemetry summary.
- CO-485 expire fallback metadata for the legacy `sandbox_mode="read-only"` compatibility retry: owner `CO-485`; trigger `active Codex CLI rejects default_permissions`; introduced date `2026-05-02`; review date `2026-05-02`; maximum lifetime `2026-06-01`; removal condition `release-facing Codex CLI pins move beyond 0.125 or the pinned CLI accepts default_permissions`; validation evidence `tests/review-launch-attempt.spec.ts` legacy retry regression plus live 0.124/0.125 command evidence. The wrapper enforces the maximum-lifetime cutoff before attempting this compatibility retry and records `legacy_fallback_*` fields in `review/telemetry.json` launch context when the fallback path is used.
...
- Operator interpretation for `review/telemetry.json`:
  ...
  - Older telemetry may not have `review_outcome`; when absent, interpret succeeded + null `termination_boundary` as clean success, failed + non-null `termination_boundary` as failed boundary, and failed + null `termination_boundary` as failed-other. Treat succeeded + non-null `termination_boundary` as a retained legacy fallback: accept it as bounded success only when the waiver/notes record owner, trigger, introduced date, review date, maximum lifetime or expiry, removal condition, reason, and validation evidence.
  ...
  - `review_verdict: unknown` or missing `review_verdict` is a review-handoff blocker unless an explicit break-glass waiver records owner, trigger, introduced date, review date, maximum lifetime or expiry, removal condition, reason, and validation evidence.
