...
- Runtime telemetry artifact: `<runDir>/review/telemetry.json` (best-effort summary of observed command activity, startup events, output tail, plus a first-class `termination_boundary` record when the current bounded command-intent, shell-probe, startup-anchor, meta-surface expansion, verdict-stability, or relevant-reinspection dwell guard fires).
