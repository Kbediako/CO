...
- Explicit wrapper scope flags (`--uncommitted`, `--base`, `--commit`) now keep the saved prompt/context in `review/prompt.txt` but launch `codex review` without any prompt argument because current Codex CLI still treats stdin (`-`) as `[PROMPT]` and rejects it with scope flags. Unscoped wrapper runs still pass the saved prompt/context inline.
- Because those explicit scoped launches cannot pass any prompt context to Codex, they support only the default `diff` surface at the actual Codex layer. `--surface audit` and `--surface architecture` must be rerun without explicit scope flags.
...
- `codex-orchestrator review`: writes task-scoped manifest evidence into the saved review prompt artifact, applies diff-budget checks when `scripts/diff-budget.mjs` exists in the target repo root (auto-skips in downstream npm installs that do not ship that helper), passes that prompt inline for unscoped launches, omits it entirely for explicit scoped launches, and fails fast if explicit scoped review also requests `--surface audit|architecture`, since those surfaces require prompt context. It may emit a handoff prompt in non-interactive mode (see above).
