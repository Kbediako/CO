# PRD - CO-330 reopened stale-owner/provider-refresh recurrence after PR #624
...
- Desired Outcome: CO-330 should now specify recurrence recovery, not just first-pass reclaim. After one fail-closed timeout restart, control-host supervision must stop rotating the host for the same active provider-worker series while provider refresh is still in a `refresh:*` phase before `restart_required`; provider refresh should then either prove fresh `control-host` and `co-status freshness`, or leave auditable `control-host-stale-owner.json` and `provider-control-host-refresh-failure.json` evidence without losing or falsely terminalizing provider refresh queue work.
...
- Success criteria / acceptance:
  ...
  - [ ] parent validation covers the observed recurrence shape from `CO-351`, `CO-352`, and `CO-355`
...
- Architectural Notes:
  ...
  - the surviving recurrence is in the supervision layer: first timeout restart remains fail-closed, while subsequent timeouts for the same active provider-worker series during active refresh should quarantine restart churn.
