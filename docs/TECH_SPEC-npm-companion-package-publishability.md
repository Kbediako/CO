# Technical Spec - Codex Orchestrator NPM Companion Package Publishability (Task 0916)
...
## Architecture & Design
### Current State
- Build output is emitted to `dist/` via `tsconfig.build.json` and includes runtime code under `dist/bin/`, `dist/orchestrator/`, `dist/packages/`, `dist/scripts/design/pipeline/`, and `dist/types/`.
- Runtime assets include `schemas/manifest.json`; no other schemas are currently required at runtime.
- Templates are stored under `templates/` and used by an explicit `init` command; `.agent/**` and other internal assets remain in-repo.
- Module system is ESM (`type: module`) with NodeNext resolution; the CLI bin must preserve the `#!/usr/bin/env node` shebang in `dist/bin`.
- User-facing MCP behavior should route through CLI subcommands instead of shell scripts in `scripts/`.
- Dev-only artifacts that must never ship include: `.agent/`, `docs/`, `tasks/`, `.runs/`, `out/`, `archives/`, `scripts/`, `tests/`, `evaluation/`, `adapters/`, `.github/`, and `node_modules/`.

### Proposed Changes
#### Package metadata + tarball allowlist
- Use `files` allowlist in `package.json` to include only:
  - `dist/bin/**`
  - `dist/orchestrator/**`
  - `dist/packages/**`
  ...
  - `README.md`
  - `LICENSE`
- Add `engines: { node: ">=20" }` and `publishConfig` (public access; provenance handled by the release workflow when OIDC is available).
- Prefer `files` allowlist over `.npmignore`.

#### Schema resolution independent of package name (Pattern A with fallback)
...
- Add `pack:audit` script that runs `npm pack --json`, parses the file list, and enforces:
  ...
  - Dist allowlist limited to runtime subtrees (example: `dist/bin/**`, `dist/orchestrator/**`, `dist/packages/**`, `dist/scripts/design/pipeline/**`, `dist/types/**`).
  - Forbidden prefixes absent (`.agent/`, `docs/`, `tasks/`, `.runs/`, `out/`, `archives/`, `scripts/`, `tests/`, `evaluation/`, `adapters/`, `.github/`, `node_modules/`).
  - Dist test artifacts rejected (`dist/**/tests/**`, `dist/**/*.test.*`, `dist/**/*.spec.*`).
...
- Add a pack smoke test that installs the `.tgz` in a temp directory and runs:
  ...
  - Use `npm install` in a temp directory (CI-only network use).

#### CLI companion surface
- Add `codex-orchestrator mcp serve` for MCP stdio:
  - Protocol output to stdout only.
  - Logs to stderr only.
  - Delegates to `codex mcp-server` with inherited stdio; enforce or verify downstream `codex` keeps logs off stdout.
...
- Release workflow runs `npm run clean:dist` before `npm run build` to avoid stale `dist/**` artifacts.
...
- Linked PRD: `docs/PRD-npm-companion-package-publishability.md`
