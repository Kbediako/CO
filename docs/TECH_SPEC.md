...
- Low-risk backlog items (Agents SDK pinning) remain tracked in task checklist Section 7 for direct implementation; the diagnostics prompt shipped 2025-10-19 with `manifest.summary` recommending `scripts/run-mcp-diagnostics.sh --no-watch`, and structured error artifacts shipped 2025-10-19 with `errors/<index>-<slug>.json` captured per failed command.
