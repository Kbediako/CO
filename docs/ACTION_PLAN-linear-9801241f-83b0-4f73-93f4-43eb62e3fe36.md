...
- Assumptions:
  ...
  - the declared source payload is absent from this child checkout
  - parent owns `docs/skills-release.md`
  - CO-466 release-intake is done and remains the version-posture authority
  - current local posture is current Codex CLI `0.128.0` with `gpt-5.5` / `xhigh`
...
- Not done if:
  ...
  - the packet reopens the full `0.128.0` release-intake lane, changes workflow pins, changes cloud-canary holds, or alters model defaults
  - package/downstream smoke pins, cloud-canary pins, broad README/book docs, or historical 0.124/0.125 evidence enter scope
...
4. Apply the smallest current-facing wording change that preserves current Codex CLI `0.128.0`, `gpt-5.5` / `xhigh`, CO-466, release-intake done, `0.125.0` release-facing hold, and `0.124.0` cloud-canary hold.
...
- Child manifest `.runs/linear-9801241f-83b0-4f73-93f4-43eb62e3fe36-packet-docs-r2/cli/2026-05-05T02-46-27-119Z-bea486de/manifest.json`.
...
- Risk: parent accidentally edits version pins or cloud canary instead of wording.
  - Mitigation: Not Done If blocks package/downstream smoke pin changes, cloud-canary pin changes, and workflow/model-default changes.
