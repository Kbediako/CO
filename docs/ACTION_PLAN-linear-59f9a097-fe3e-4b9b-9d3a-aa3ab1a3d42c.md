...
- Assumptions: the shared source-0 payload in this child lane is metadata/provenance only; the full issue body was read through the packaged read-only `linear issue-context` helper; parent owns all source/test work.
...
7. Parent proves genuine stuck refreshes still emit `provider_refresh_lifecycle_stuck` and `restart_required=true`.
8. Parent proves recovery preserves active provider workers and request-budget/no-request-burn safeguards from `CO-163` / `CO-179`.
9. Parent verifies post-recovery `co-status --format json` with `polling.stuck=false`, `polling.restart_required=false`, and live provider workers visible.
10. Parent runs focused regression coverage for repeated churn and a no-regression path for `CO-194`, then completes docs-review, review, and PR handoff.
