...
- Accepted same-issue docs lane: `agent-task-restore` restored `.agent/task/linear-994efebc-e1e4-4e00-8046-c60143813251.md` at `.runs/linear-994efebc-e1e4-4e00-8046-c60143813251-agent-task-restore/cli/2026-04-21T20-59-07-638Z-db747213/manifest.json`; parent owns the remaining docs, source, tests, validation, and PR work.
...
- Risk: parent and child scopes collide during the Rework reset.
  - Mitigation: keep the accepted child lane narrowly scoped to `.agent/task`, park parent-owned source/test changes while the lane is active, then reapply the parent diff after acceptance.
