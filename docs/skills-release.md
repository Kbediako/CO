...
- `npm run pack:smoke` is the standard downstream simulation gate:
  ...
  - validates packaged Codex marketplace install/register coverage for `codex-orchestrator` through Codex `marketplace add` and plugin install/status checks.
...
- CI standard: core-lane runs `npm run pack:smoke` automatically when downstream-facing paths change (CLI/package/skills/review-wrapper/docs wiring). Core-lane, pack-smoke backstop, and release workflows install `@openai/codex@0.121.0` before `npm run pack:smoke` so marketplace coverage is present in release-facing lanes. A newer Codex CLI may replace that marketplace pin only after it exposes `codex marketplace add` and `npm run pack:smoke` passes without `PACK_SMOKE_ALLOW_MARKETPLACE_SKIP`.
