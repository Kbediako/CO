...
- Marketplace command transition: Codex CLI `0.121.0` accepts both `codex marketplace add` and `codex plugin marketplace add`; Codex CLI `0.122.0+` requires `codex plugin marketplace add`; current local posture is Codex CLI `0.135.0` with CO-local `gpt-5.5` / `xhigh`, and that CLI still supports `codex plugin marketplace upgrade` and `remove`.
