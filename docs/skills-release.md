...
- CI standard: core-lane runs `npm run pack:smoke` automatically when downstream-facing paths change (CLI/package/skills/review-wrapper/docs wiring). Core-lane, pack-smoke backstop, and release workflows install `@openai/codex@0.124.0` before `npm run pack:smoke` so plugin-marketplace coverage is present in release-facing lanes.
- `cloud-canary` also installs `@openai/codex@0.124.0`, but it is the cloud execution/fallback evidence lane, not release-facing `pack:smoke` marketplace proof.
