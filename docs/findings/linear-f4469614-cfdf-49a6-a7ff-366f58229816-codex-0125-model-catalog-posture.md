...
| Required cloud canary | `CODEX_CLOUD_CANARY_REQUIRED=1 npm run ci:cloud-canary` failed with `environment '<configured-env-id>' not found`. | Failed required cloud gate. | Blocks cloud execution and release-facing pin promotion only; it is not evidence to keep CO-local posture on `gpt-5.4`. |
