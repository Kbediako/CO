...
- Orchestrator env resolution now uses shared run-manifests helpers (`resolveEnvironmentPaths`, `listDirectories`); status UI build + ExperienceStore defaults honor the same resolver, and the helpers ship in `dist/`.
