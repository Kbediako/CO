...
- `1037` isolated selected-run payload construction into the then-current selected-run presenter seam; `CO-88` later removed that dead file after `/ui` moved to `operatorDashboardPresenter.ts`.
...
- Keep the then-current UI dataset builder unchanged within this slice.
