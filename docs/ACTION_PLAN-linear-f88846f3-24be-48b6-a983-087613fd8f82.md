...
- [ ] Reproduce the fixture-audit leak shape under a provider-worker audit environment.
- [ ] Isolate mutating Linear CLI test audit writes from the live provider audit path while preserving real audit logging.
- [ ] Add focused regressions and run the full provider-worker validation gate set.
