...
- [Operations](operations.md): common commands, run artifacts, workflow modes, permission/trust posture, and review or merge handoff expectations
...
- [Historical Codex CLI 0.124.0 Evidence](archive/codex-cli-0124-adoption.md): archive-only CO-341/CO-345 evidence for the `0.124.0` step; see the canonical version policy for the current `0.135.0` ChatGPT-auth appserver/model posture, package/downstream-smoke `0.125.0` compatibility hold, and cloud-only `0.124.0` candidate split. CO-579 observed local `0.140.0` smoke evidence, but that is not a canonical posture promotion.
