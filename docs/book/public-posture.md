...
CO currently uses Codex CLI `0.125.0` for validated local ChatGPT-auth/appserver posture and marketplace/downstream-smoke compatibility. Cloud execution, provider-supervision replacement, and broader release promotion remain separately gated by the canonical version policy.
...
- Local `gpt-5.5` use is the current CO posture after live access smoke; use the `gpt-5.4` fallback only when access or portability is unavailable.
