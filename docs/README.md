...
- **Advanced feature posture:** Upstream `rust-v0.128.0` removed `js_repl` and `js_repl_tools_only`, so CO no longer treats `js_repl` as default-on, break-glass toggleable, or eligible for cloud feature-contract lanes. Use `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` only for active non-removed feature names after checking `codex features list`; `memories` remains scoped to explicit eval lanes (legacy alias `memory_tool` is compatibility-only).
