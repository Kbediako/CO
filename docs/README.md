...
## Current Posture
- Current CO-local ChatGPT-auth/appserver model posture: `gpt-5.5` / `xhigh` on Codex CLI `0.130.0` when live access smoke passes.
...
- **Advanced feature posture:** Upstream `rust-v0.128.0` removed `js_repl` and `js_repl_tools_only`, and current local `0.130.0` still reports both as removed, so CO no longer treats either as default-on, break-glass toggleable, or eligible for cloud feature-contract lanes. Do not set `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` to `js_repl` or `js_repl_tools_only`, and do not run `codex features enable/disable` for either removed feature. Use `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` only for active non-removed feature names after checking `codex features list`; `memories` remains scoped to explicit eval lanes (legacy alias `memory_tool` is compatibility-only).
