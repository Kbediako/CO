...
Historical note: the current repo no longer ships the token/component publication surface described below. `packages/design-system` is now only a placeholder scaffold with helper scripts, so use this section as a historical capture-process reference rather than a current package capability promise.
