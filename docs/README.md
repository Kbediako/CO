...
## Current Posture
- Current CO-local ChatGPT-auth/appserver model posture: `gpt-5.5` / `xhigh` on Codex CLI `0.135.0` when live access smoke passes.
- Release-facing package/downstream-smoke compatibility targets Codex CLI `0.125.0`; `cloud-canary` remains on the explicit `0.124.0` cloud execution candidate until the configured-environment blocker is cleared and fallback cloud evidence is clean.
...
- **Advanced feature posture:** Upstream `rust-v0.128.0` removed `js_repl` and `js_repl_tools_only`, and current local `0.135.0` still reports both as removed, so CO no longer treats either as default-on, break-glass toggleable, or eligible for cloud feature-contract lanes. `goals` now reports `stable true` on `0.135.0`; provider-worker goal derivation/status work should consume goals as stable while preserving explicit compatibility behavior for older evidence. Do not set `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` to `js_repl` or `js_repl_tools_only`, and do not run `codex features enable/disable` for either removed feature. Use `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` only for active non-removed feature names after checking `codex features list`; `memories` remains scoped to explicit eval lanes (legacy alias `memory_tool` is compatibility-only).
