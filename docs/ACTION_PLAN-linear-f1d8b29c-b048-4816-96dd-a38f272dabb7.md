...
- Scope: release proof, generated schema capture, local app-server smoke, runtime canary, provider parity matrix, hold decision, validation, standalone review, elegance pass, and PR handoff.
- Boundary: no runtime switch, no authority expansion, no provider precedence change, no fallback removal.

## Sequence
1. Confirm Linear state and workpad, then move active issue into `In Progress`.
...
4. Draft PRD, TECH_SPEC, ACTION_PLAN, task checklist, docs findings, and registry updates.
5. Refresh release proof from npm, GitHub tags, and local binary.
6. Run app-server schema generation, stdio initialize smoke, and isolated runtime canary.
