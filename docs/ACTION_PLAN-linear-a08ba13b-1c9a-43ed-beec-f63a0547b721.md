...
- Scope: CO-543 packet, thirteen stale `tasks/specs/**` rows, task-index/registry evidence, related terminal packet registry rows, `docs/TASKS.md`, active spec pre-expiry surfacing for frontmatter and legacy `last_review:` specs, legacy `tasks[]` lifecycle parity with `spec-guard`, and focused owner-action routing coverage.
...
8. Add focused prevention assertions for active spec pre-expiry surfacing, legacy `last_review:` parity, inactive frontmatter formatting parity, legacy `tasks[]` terminal lifecycle parity with `spec-guard`, archived-stub exclusion, and owner-action routing on over-budget freshness debt.
