...
## Summary
- Objective: make explicit commit/base standalone-review launches execute truthfully by moving scoped Codex review prompt delivery from unsupported argv payloads to stdin while keeping wrapper-side evidence intact.
- Scope:
...
- Constraints:
  ...
  - preserve prompt/context artifacts while scoped review delivers the same prompt via stdin instead of inline argv
  - stay out of unrelated review-policy or provider-worker workflow changes

## Technical Requirements
- Functional requirements:
  - explicit `--commit` review runs must omit the inline prompt argument passed to `codex review` and stream the prompt via stdin
  - explicit `--base` review runs must omit the inline prompt argument passed to `codex review` and stream the prompt via stdin
  - wrapper help/docs must explain that explicit scoped runs keep prompt/context in saved artifacts and feed the same prompt to Codex via stdin rather than inline argv
...
- Architecture / design adjustments:
  ...
  - split prompt artifact persistence from prompt transport so explicit scoped launches can swap argv delivery for stdin
...
- Rollout verification:
  - confirm scoped launch args no longer include the prompt payload inline and instead append stdin marker `-`
  - confirm scoped prompt artifacts still render path-only scope notes and evidence context and that the same prompt is available for stdin transport
