...
### Data Flow
1. Completion detected → Harvester tags snapshot and records `{TagName, SnapshotCommitSHA, TarballDigest}`; if tarball used, copies the current working tree (tracked + untracked) to `learning-snapshots/<task-id>/<run-id>.tar.gz` under the runs root with a 30-day retention guideline.
