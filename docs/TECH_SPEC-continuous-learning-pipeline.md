...
- **Crystalizer:** Use an LLM helper (provider: OpenAI `gpt-4o`, prompt pack `prompt-packs/crystalizer-v1` with stamped hash) to produce Markdown patterns from validated patches + PRD context, writing to `.agent/patterns/candidates/` with `Problem/Solution/Rationale` sections. Per-run token/cost budget: $0.50 max; abort and alert if exceeded.
