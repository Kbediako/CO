...
- **Crystalizer:** Use an LLM helper (provider: `gpt-5.1-codex-max` by default, prompt pack `prompt-packs/crystalizer-v1` with stamped hash) to produce Markdown patterns from validated patches + PRD context, writing to `.agent/patterns/staging/` with `Problem/Solution/Rationale` sections. Per-run token/cost budget: $0.50 max; abort and alert if exceeded. Model may be overridden via `CRYSTALIZER_MODEL`. A Codex CLI-backed client is available (`createCodexCliCrystalizerClient`) for environments that rely on Codex login instead of OpenAI API keys.
...
- **Governance:** Keep `.agent/patterns` as canonical; candidates remain in `staging/` until reviewed/promoted to `active/` or moved to `deprecated/`. Repo docs may link only.
...
6. Successful validation produces best patch + signals; crystalizer emits pattern to `.agent/patterns/staging/` and links back to run manifest. Validation logs remain at `learning.validation.log_path`.
7. Librarian/reviewer promotes or deprecates patterns inside `.agent/patterns`, with optional doc links in `docs/`.
...
- Patterns stored only under `.agent/patterns/{staging,active,deprecated}/`; any docs entry must point back to the canonical file.
...
## Testing Strategy
- Unit / Integration: Cover harvester snapshot tagging + metadata persistence, queue payload shape, runner snapshot verification, heuristic scenario synthesis (including fallbacks), manual requeue gating, and crystalizer output format to `.agent/patterns/staging/`.
