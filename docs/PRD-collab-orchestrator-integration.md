...
- Treat collab as additive for intra-run multi-agent collaboration (planning splits, RLM symbolic subcalls, subagent brainstorms) and enable it explicitly (`--enable multi_agent` / `RLM_SYMBOLIC_MULTI_AGENT=1`; legacy aliases: `--enable collab`, `RLM_SYMBOLIC_COLLAB=1`).
