...
## Summary
- Objective: require autonomous merged closeout to inspect the shared CO root checkout, fast-forward it to `origin/main` only when safe, and otherwise record an explicit skip reason in the final workpad closeout.
- Scope:
...
- Architecture / design adjustments:
  - extend `Merging` guidance in the provider-worker prompts so “move to Done” becomes “reconcile or explicitly skip the shared root, then move to Done”
  - extend the `linear` and `land` skills to carry the same contract for workpad evidence and safe-sync gating
...
- External dependencies / integrations:
  - git CLI for shared-root status/fetch/fast-forward commands
...
## Approvals
- Reviewer: docs-review approved via `.runs/linear-156d7133-00ba-40be-bd35-67cd3ae46e21/cli/2026-03-27T22-20-59-833Z-8c11f043/manifest.json`
- Date: 2026-03-28
