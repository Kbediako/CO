...
- Success criteria / acceptance: document or classify `features.multi_agent_v2.max_concurrent_threads_per_session`, keep `agents.max_threads` rejected/omitted under v2, preserve stable `[agents] max_threads = 12`, and prove old-path rejection plus new-path acceptance with focused tests or probes.
...
| New v2 cap path | CO has no first-class guidance for `features.multi_agent_v2.max_concurrent_threads_per_session`. | Codex CLI 0.128 accepts the feature-specific cap path. | Docs and doctor/default/init posture either support it or clearly classify it as user-owned. |
