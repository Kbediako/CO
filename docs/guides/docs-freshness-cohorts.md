...
The CO-558 passes directly reviewed strict pre-expiry rows instead of adding them to rolling deferral. The release-intake issue template, public book guides, `skills/README.md`, and the shipped `agent-first-adoption-steering` and `long-poll-wait` skills still match current provider-worker guidance after review. The May 20 refresh also re-reviewed twelve active spec pre-expiry rows and refreshed their registry metadata from explicit review evidence while leaving legacy spec source files unchanged when no content update was needed.
