...
The CO-558 passes directly reviewed strict pre-expiry rows instead of adding them to rolling deferral. The release-intake issue template, public book guides, `skills/README.md`, and the shipped `agent-first-adoption-steering` and `long-poll-wait` skills still match current provider-worker guidance after review. The May 20 pass also found twelve pre-expiry spec rows whose Linear issues are already `Done`; those completed-lane task packets are archived through `tasks/index.json` and `docs/docs-freshness-registry.json` lifecycle metadata instead of blindly refreshing legacy fallback spec frontmatter.
