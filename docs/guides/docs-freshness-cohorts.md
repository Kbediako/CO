...
- Global owner issue metadata: `CO-579` is the active recurring maintenance owner binding. If it is terminal while retained debt still resolves to it, automation must emit `restore_existing_owner` / `move_to_backlog_not_done` so the same owner returns to `Backlog`; terminal `CO-575`, terminal `CO-573`, and terminal `CO-558` are retained as historical evidence, and current retained-cohort ownership is routed through exact `canonical_owner_issues[]` overrides where they exist.
- Owner lifecycle metadata: owner bindings must declare whether they are `active_owner`, `retiring`, or `retired_historical`; only `active_owner` / `retiring` bindings can be restored or used as live owner proof, while `retired_historical` is lineage only.
- Canonical recurring owner key: `docs:freshness:maintain`
- Live-owner verification: `docs:freshness:maintain` must verify the configured owner issue as a non-terminal issue in the configured Linear project before owned rolling debt can pass. Terminal active owners route to restore action when restoration is possible; retired historical owners are lineage only and cannot satisfy live owner proof.
- Exact canonical owner overrides: `canonical_owner_issues[]` may map one `canonical_owner_key` to one live owner issue, such as `CO-581` for `baseline_cohort_id:co-558-may-19-apr-18-task-report-maintenance`, `CO-569` for `baseline_cohort_id:co-558-may-20-apr-19-task-report-maintenance`, or `CO-320` for `docs_freshness_candidate|doc_class:task_packet|path_family:tasks/tasks-*|last_review:2026-03-23|cadence_days:30`
- Historical owner lineage: `CO-175` established the Apr 14 baseline, `CO-267` owned the Apr 20/21 maintenance refreshes, `CO-300` owned the Apr 22 reset, `CO-324` owned the Apr 23 reset, `CO-343` owned the Apr 24/25 reset, `CO-401` owned the Apr 27 reset, `CO-420` temporarily re-homed the Mar 28 rolling cohort before reaching terminal `Done`, `CO-409` restored live same-project owner metadata after `CO-420` before also reaching terminal `Done`, `CO-423` restored live same-project owner metadata after `CO-409` before reaching terminal `Done`, `CO-425` restored live same-project owner metadata after `CO-423` before reaching terminal `Done`, `CO-430` briefly carried the integrated owner metadata from the CO-428 blocker-clear branch before also reaching terminal `Done`, `CO-427` restored live same-project owner metadata after `CO-425`/`CO-430` before reaching terminal `Done`, `CO-441` restored live same-project owner metadata after `CO-427` before reaching terminal `Done`, `CO-444` restored live same-project owner metadata after `CO-441` before reaching terminal `Done`, `CO-511` restored live same-project owner metadata after `CO-444` before reaching terminal `Done`, `CO-522` restored live same-project owner metadata after `CO-511` before later reaching terminal `Done`, `CO-558` restored the May 19/20 rolling freshness maintenance track before also reaching terminal `Done`, `CO-568` restored the May 19 exact-key cohort before reaching terminal `Done`, `CO-573` restored live same-project owner metadata after terminal `CO-558` before reaching terminal `Done`, `CO-575` restored live same-project owner metadata after terminal `CO-573` before reaching terminal `Done`, and `CO-579` restores live same-project owner metadata after terminal `CO-575`; these prior owners are now terminal or invalid evidence only and must not remain the live maintenance owner after they reach terminal or unverifiable states. `CO-581` and `CO-569` are current live same-project exact-key owners for the retained May 19 and May 20 cohorts respectively.
...
`owner_finalizer` is the terminal-closeout contract. When active freshness candidates resolve to a global owner or exact cohort owner, `docs:freshness:maintain` records every resolved active owner in `active_owner_issues[]` and `active_owner_records[]` while keeping `active_owner_issue` as a compatibility summary. A terminal same-project `active_owner` must emit `status=restore_existing_owner` with `reason=move_to_backlog_not_done` so operators move it back to `Backlog` instead of creating another replacement owner; terminal `retired_historical`, wrong-project, or otherwise unusable owners must still fail closed. Provider merge closeout must block `Done` for any active owner issue instead of letting that owner become terminal while the debt still resolves to it. `not_applicable` means no active freshness candidates currently resolve to an owner; it is not evidence that historical terminal owners can be reused later.

Before an owner is usable, automation must verify live owner state. Open same-project issues with the exact marker are reused and updated. Terminal active owners are restored to `Backlog` when possible; terminal retired historical owners, canceled, duplicate, out-of-project, or state-unverified owners are evidence only and require replacement action. Missing exact-marker owners must emit create-action evidence. Dry-run and no-token paths must not mutate Linear, but they must still emit copyable restore/create/update bodies in `owner_action_evidence.actions[]` so the parent/provider lane can apply the action without re-inventing the issue text.
...
When the configured owner issue is terminal, canceled, duplicate, out-of-project, or unverifiable, `docs:freshness:maintain` must fail closed. Terminal same-project `active_owner` metadata is a restoration action, not proof of clean ownership; terminal `retired_historical` or mismatched owner metadata is evidence only. The helper must require canonical `docs:freshness:maintain` owner reuse, Backlog restoration, or a new live same-project owner issue instead of reusing a stale `Done`, `Duplicate`, `Canceled`, or project-mismatched owner path. The maintenance output must keep the owner action machine-readable so follow-up creation can pass `--canonical-owner-key docs:freshness:maintain` or restoration can run an audited `linear transition --state "Backlog" --force` without creating duplicate recurring baseline owners.
...
- restore a terminal same-project `active_owner` by moving it to `Backlog` with `move_to_backlog_not_done`
- replace a `retired_historical`, canceled, duplicate, out-of-project, or state-unverified owner with a new exact-marker owner action
- create a same-project owner when no exact-marker owner exists
- in dry-run or no-token execution, copy the emitted restore/create/update body exactly rather than summarizing it
...
The May 20 date rollover added `co-558-may-20-apr-19-task-report-maintenance` under the same live owner. The `2026-04-19` task mirror, task packet, and report-only rows also keep their original `last_review` evidence and remain visible in `rolling_freshness_cohorts` until the rolling window expires after `2026-05-26`.

After `CO-558` reached terminal `Done`, CO-552 created exact-key replacement owners `CO-568` and `CO-569` for the May 19 and May 20 retained cohorts. After `CO-568` also reached terminal `Done`, `CO-581` re-homes the May 19 exact-key owner while keeping `CO-568` as configured historical terminal-owner evidence. The re-home is represented only as `canonical_owner_issues[]` overrides: the historical cohort ids and original `last_review` evidence remain unchanged, and both cohorts still expire on their original dates unless refreshed, archived, reclassified, or re-homed again.

After current-main maintenance evidence again showed global `docs:freshness:maintain` routing through terminal `CO-558`, CO-573 re-homes only the global live owner metadata to same-project issue `CO-573`. The exact-key May 19 and May 20 cohort overrides continue to point at live `CO-581` and `CO-569`, and the CO-558/CO-568 cohort ids, original `last_review` values, strict pre-expiry review evidence, and terminal-owner lineage remain machine-visible rather than being deleted, blindly refreshed, or reassigned to CO-572.

After May 22 maintenance evidence again showed global `docs:freshness:maintain` routing through terminal `CO-573`, CO-575 re-homes only the global live owner metadata to same-project issue `CO-575`. The exact-key May 19 and May 20 cohort overrides continue to point at live `CO-581` and `CO-569`, and the CO-573/CO-558/CO-568 owner lineage, original `last_review` values, strict pre-expiry review evidence, terminal-owner replacement evidence, completed-lane registry residue, stale active-spec routing, fallback/seam metadata routing, and dry-run/no-token copyable body remain machine-visible rather than being deleted, blindly refreshed, or reassigned to CO-567, CO-570, or CO-574.

The CO-558 passes directly reviewed strict pre-expiry rows instead of adding them to rolling deferral. The release-intake issue template, public book guides, `skills/README.md`, and the shipped `agent-first-adoption-steering` and `long-poll-wait` skills still match current provider-worker guidance after review. The May 20 refresh also verified twelve spec pre-expiry rows whose source issues are terminal; the spec frontmatter records that terminal disposition, so maintenance can skip active-spec pre-expiry action without editing legacy fallback content or archiving packet docs.
