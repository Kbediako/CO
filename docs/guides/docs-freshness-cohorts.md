...
- Global owner issue metadata: `CO-579` is the live recurring maintenance owner; terminal `CO-575`, terminal `CO-573`, and terminal `CO-558` are retained as historical evidence, and current retained-cohort ownership is routed through exact `canonical_owner_issues[]` overrides where they exist.
