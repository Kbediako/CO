...
## Apr 29 Terminal CO-425 Owner Re-home

### Reproduction / Baseline Findings
CO-427 reproduced the owner blocker after CO-425 reached terminal `Done`:
...
- Live Linear verification showed `CO-425` as a same-project terminal `Done` issue, while `CO-427` is the live same-project `docs:freshness:maintain` owner lane.
- The retained cohort remains `co-420-apr-28-march-28-task-packet-mirror`: `33` rolling rows, split into `28` Task Packet rows and `5` Task Mirror rows, with `last_review=2026-03-28`, `cadence_days=30`, and `expires_after=2026-05-04`.
