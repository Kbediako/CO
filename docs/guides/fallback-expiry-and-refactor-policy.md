...
- Docs freshness ownership: `scripts/docs-freshness-maintain.mjs` and `docs/guides/docs-freshness-cohorts.md` treat owned rolling debt as an `expire fallback`: owner metadata must verify as a live same-project owner, fallback expiry metadata must name the review/removal condition, and stale or unverifiable owner metadata must route through canonical owner key `docs:freshness:maintain`.
