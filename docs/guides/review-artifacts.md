...
cat "$run_dir/review/telemetry.json"
```

- If you already know the task and run-id, skip the search and work off that exact path.
- When you need to hand review evidence to a reviewer, zip `"$run_dir/review"` or reference the relative paths printed by `codex-orchestrator review`.
