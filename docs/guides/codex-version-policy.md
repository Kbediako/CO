...
- Release-facing downstream-smoke workflows (`core-lane`, `release`, and `pack-smoke-backstop`) stay pinned to `@openai/codex@0.121.0` because `pack:smoke` requires `codex marketplace add`, which remains available in `0.121.0` but is absent from the local `0.122.0` marketplace surface.
- `cloud-canary` pins `@openai/codex@0.122.0` as the explicit audited candidate so canary evidence stays reproducible without forcing marketplace-dependent smoke lanes onto an incompatible CLI surface.
...
- 2026-04-21: `CO-269` directly compared marketplace capability across the workflow-candidate versions. `npx --yes @openai/codex@0.121.0 marketplace --help` exposes `add`, while `npx --yes @openai/codex@0.122.0 marketplace --help` falls back to top-level Codex help and does not expose the marketplace surface. Because `scripts/pack-smoke.mjs` fails closed when `codex marketplace add` is unavailable, downstream marketplace smoke workflows stay pinned to `0.121.0` in this lane. Evidence: `out/linear-a2949fd4-2319-4cd0-acd1-68e6404766f3/manual/codex-0122-command-surface/marketplace-capability-compare.log`, `scripts/pack-smoke.mjs`, and `tests/pack-smoke.spec.ts`.
- 2026-04-21: `CO-269` classified `rust-v0.122.0` deltas for CO as follows:
  ...
  - Hold: release-facing downstream-smoke workflows remain on `@openai/codex@0.121.0` until a separate marketplace follow-up proves `0.122.0` can satisfy the `pack:smoke` `codex marketplace add` contract or intentionally revises that contract with evidence.
...
- 2026-04-21: `CO-275` refreshed the marketplace comparison and closed the post-`0.121.0` marketplace policy question for now: keep `codex marketplace add` as the `pack:smoke` prerequisite, keep release-facing downstream-smoke workflows pinned to marketplace-capable `@openai/codex@0.121.0`, and allow a newer Codex CLI replacement only after reproducible evidence shows `npx --yes @openai/codex@<candidate> marketplace add --help` succeeds and `npm run pack:smoke` passes without `PACK_SMOKE_ALLOW_MARKETPLACE_SKIP`. Evidence: `out/linear-09c996ec-47b3-4f55-928d-38e0b17c11df/manual/codex-marketplace-policy/marketplace-capability-compare.log`, `scripts/pack-smoke.mjs`, `tests/pack-smoke.spec.ts`, `.github/workflows/core-lane.yml`, `.github/workflows/release.yml`, and `.github/workflows/pack-smoke-backstop.yml`.
