...
- 2026-04-21: `CO-275` closes the post-`0.121.0` marketplace-dependent downstream-smoke policy question by accepting the `CO-268` replacement contract as the current long-term policy: `pack:smoke` remains mandatory marketplace coverage, but the proof path is now `@openai/codex@0.122.0` plus `codex plugin marketplace add` instead of the historical `@openai/codex@0.121.0` plus `codex marketplace add` hold from `CO-269`. The old `0.121.0` evidence remains lineage for `CO-217` mandatory-coverage semantics and does not reopen active-target promotion.
