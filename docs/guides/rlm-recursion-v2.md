...
- Codex CLI `0.128+` exposes the MultiAgentV2-specific thread cap at `features.multi_agent_v2.max_concurrent_threads_per_session`. Treat it as v2-only user-owned tuning: enable v2 explicitly (for example `-c features.multi_agent_v2=true`) and set the cap with `-c features.multi_agent_v2.max_concurrent_threads_per_session=<n>` when needed; do not move stable `features.multi_agent=true` users off the `[agents] max_threads = 12` baseline.
