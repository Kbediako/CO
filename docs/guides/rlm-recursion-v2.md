...
- For Codex CLI `0.125+` with effective `features.multi_agent_v2=true`, omit `agents.max_threads` entirely; upstream rejects the key, so doctor/default setup must not write or recommend it.
- Codex CLI `0.128+` exposes the MultiAgentV2-specific thread cap on the `multi_agent_v2` config surface. Treat persisted `[multi_agent_v2] max_concurrent_threads_per_session = <n>` as v2-only user-owned tuning; the feature-specific CLI override/probe path `features.multi_agent_v2.max_concurrent_threads_per_session` remains explicit user-owned evidence for 0.128+ acceptance. Do not move stable `features.multi_agent=true` users off the `[agents] max_threads = 12` baseline.
- `features.multi_agent` is canonical for v1/older collab tooling; `features.multi_agent_v2` is the Codex CLI `0.125+` path that rejects `agents.max_threads` and, in `0.128+`, owns the v2-specific cap path; `collab` is a legacy alias.
