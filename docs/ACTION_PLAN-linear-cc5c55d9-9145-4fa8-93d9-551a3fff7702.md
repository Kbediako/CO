...
- Intent checksum / protected terms carried forward: preserve .git/config, `git config --worktree`, `extensions.worktreeConfig`, `inherit global identity by default`, `no shared [user] override`, and `cloud / Codex-upgrade hypothesis is not the root cause`.
...
1. Create the CO-120 docs packet, registry entries, checklist mirrors, and local workpad source; upsert the single workpad comment.
...
## Validation
- Checks / tests:
  - `MCP_RUNNER_TASK_ID=linear-cc5c55d9-9145-4fa8-93d9-551a3fff7702 node "/Users/kbediako/Code/CO/dist/bin/codex-orchestrator.js" linear child-stream --pipeline docs-review --stream co-120-docs-review --format json`
...
- Risk: the lane drifts into history rewriting or global git policy.
  - Mitigation: keep the scope fixed on future linked-worktree behavior and proof only.

## Approvals
- Reviewer: `docs-review` child streams recorded at `.runs/linear-cc5c55d9-9145-4fa8-93d9-551a3fff7702-co-120-docs-review-rerun/cli/2026-04-09T08-46-05-901Z-e6964d2e/manifest.json` and `.runs/linear-cc5c55d9-9145-4fa8-93d9-551a3fff7702-co-120-docs-review-approval-rerun/cli/2026-04-09T08-59-49-479Z-b1d984dc/manifest.json`; the approval rerun is accepted via manual fallback after `docs:check` / `docs:freshness` succeeded and the final packet wording issue was fixed. Follow-up implementation review is still required after code changes.
- Date: 2026-04-09
