...
- Assumptions:
  - the smallest durable fix is to keep prompt artifacts while switching explicit scoped review launches from inline argv prompt delivery to stdin delivery
...
3) Patch the shared launch builder and any direct telemetry/help seams so explicit commit/base review runs stop passing unsupported inline prompt payloads, stream the prompt over stdin, and keep wrapper-side evidence intact.
...
- Checks / tests:
  ...
  - `npm run pack:smoke`
- Rollback plan:
  - revert scoped launch changes if they silently drop scope, lose prompt artifact evidence, or fail to stream the saved prompt via stdin
  - revert telemetry/help/doc wording if it misstates whether scoped launches use inline argv or stdin delivery

## Risks & Mitigations
- Risk: scoped launch succeeds but docs/telemetry still misstate whether prompt delivery happens inline or via stdin.
  - Mitigation: update direct guidance surfaces in the same patch and add focused assertions on saved evidence plus scoped stdin delivery.
