...
- [x] Checklist mirrored to `.agent/task/linear-8bbdd424-d77e-4312-b4b7-2a82c3df2749.md`. Evidence: this file.
...
- [ ] Released/non-active claims no longer surface `run_manifest_path` / run-state metadata that contradict live issue state or terminal run outcome. Evidence: pending.
- [ ] Terminal issues do not retain pointers to `in_progress` manifests in operator-facing intake/state surfaces. Evidence: pending.
- [ ] Operator autopilot unblock recommendations are suppressed or corrected when blocker truth is stale or contradicted by fresher issue/PR state. Evidence: pending.
- [ ] `linear-advisory-state.json` is refreshed truthfully from live source or explicitly marked deprecated/stale. Evidence: pending.
- [ ] Active-lane admission/attach behavior remains preserved. Evidence: pending.
