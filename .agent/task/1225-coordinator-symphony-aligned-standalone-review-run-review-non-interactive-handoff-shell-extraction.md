...
- [ ] Non-interactive handoff shell extracted behind a dedicated helper/module. Evidence: implementation closeout summary and shipped helper/module paths.
- [ ] `run-review.ts` remains the orchestration owner for prompt context, boundary preflight, runtime invocation, live monitor policy, and final telemetry behavior. Evidence: shipped wrapper/orchestration paths.
- [ ] Focused regressions cover artifact creation, env export, and printed handoff suppression behavior. Evidence: implementation closeout summary and focused regression logs.
