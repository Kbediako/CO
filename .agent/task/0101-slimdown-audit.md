...
4) Delete `scripts/mcp-runner-migrate.js` + `scripts/mcp-runner-metrics.js` and update `.runs/README.md` + `docs/REFRACTOR_PLAN.md`.
...
- [ ] Phase 2 consolidations executed (helpers and legacy scripts).
  - [ ] Replace duplicate `writeJsonAtomic` helpers (`scripts/status-ui-build.mjs`, `packages/shared/design-artifacts/writer.ts`) with `orchestrator/src/cli/utils/fs.ts`.
  - [ ] Replace local `sanitizeTaskId` / `sanitizeRunId` in `packages/shared/design-artifacts/writer.ts` with orchestrator helpers.
  - [ ] Standardize env path resolution on `CODEX_ORCHESTRATOR_ROOT` (remove `CODEX_ORCHESTRATOR_REPO_ROOT` usage).
  - [ ] Remove `scripts/mcp-runner-migrate.js` + `scripts/mcp-runner-metrics.js` and update `.runs/README.md`.
...
- [x] Implementation-gate manifest captured after code changes - Evidence: `.runs/0101-slimdown-audit/cli/2026-01-01T05-57-43-325Z-cf23c380/manifest.json`.
- [x] Diff budget override recorded (`DIFF_BUDGET_OVERRIDE_REASON`) - Evidence: `.runs/0101-slimdown-audit/cli/2026-01-01T05-57-43-325Z-cf23c380/manifest.json`.
