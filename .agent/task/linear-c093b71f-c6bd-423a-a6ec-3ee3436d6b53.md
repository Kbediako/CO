...
- Current recovery validation: `node scripts/spec-guard.mjs --dry-run`, `npm run build`, `npm run lint`, `npm run test` (364 files / 6004 tests), `npm run docs:check`, `npm run docs:freshness`, `npm run repo:stewardship`, `node scripts/diff-budget.mjs`, `npm run pack:smoke`, and `codex-orchestrator hygiene quota --format json` passed on latest `origin/main`; review rework focused tests passed 24 quota hygiene cases.
