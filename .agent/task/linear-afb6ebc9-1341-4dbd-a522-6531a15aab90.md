...
## Validation
- [x] Focused eval tests pass for source/prompt-pack adoption, source_path provenance matching, child-lane lifecycle proof, direct script execution, and follow-up/link/workpad traceability. Evidence: `npx vitest run --config vitest.config.ts evaluation/tests/provider-linear-adoption.test.ts` (`7/7` tests), `npm run eval:provider-adoption`, `out/linear-afb6ebc9-1341-4dbd-a522-6531a15aab90/provider-linear-adoption-eval.json`.
