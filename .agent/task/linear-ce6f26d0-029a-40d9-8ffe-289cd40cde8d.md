...
- [ ] Full validation floor is green after the merge commit is recorded. Evidence: first `npm run test` attempt was blocked by `tests/run-review.spec.ts` provider-worker review artifact rebasing subprocesses because the branch was still in an uncommitted merge state and the wrapper self-test hit its large uncommitted-scope guard; rerun from a clean committed branch is pending.
- [ ] Workpad and closeout artifacts record the final controller shape, adopted consumer surfaces, validation, review/elegance status, PR attachment, and ready-review drain. Evidence: pending final refresh.
...
- [ ] Latest `origin/main` is merged into the branch before review-state transition. Evidence: merge conflicts resolved locally against `origin/main` (`a8a1ff9bb`); merge commit pending.
