# Task Checklist — Coordinator Symphony-Aligned Orchestrator Auto-Scout Evidence Recorder Extraction (1167)

> Set `MCP_RUNNER_TASK_ID=1167-coordinator-symphony-aligned-orchestrator-auto-scout-evidence-recorder-extraction` for orchestrator commands. Mirror status with `tasks/tasks-1167-coordinator-symphony-aligned-orchestrator-auto-scout-evidence-recorder-extraction.md` and `docs/TASKS.md`. Flip `[ ]` to `[x]` only with manifest evidence (e.g., `.runs/1167-coordinator-symphony-aligned-orchestrator-auto-scout-evidence-recorder-extraction/cli/<run-id>/manifest.json`).
