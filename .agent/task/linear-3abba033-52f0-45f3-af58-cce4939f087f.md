...
- [x] `npm run docs:freshness` ran and failed only on the existing repo-wide stale-doc baseline.
- [x] `npm run repo:stewardship`.
- [x] `node scripts/diff-budget.mjs`.
- [x] Manifest-backed standalone review: final telemetry reports `review_verdict=clean`, `finding_count=0`.
