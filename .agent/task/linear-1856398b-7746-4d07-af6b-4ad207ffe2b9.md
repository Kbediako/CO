...
- [x] Parent adds focused docs-hygiene tests for stale active release evidence residue, including the `0.124` book-style case. Evidence: `npx vitest run --config vitest.config.core.ts tests/docs-hygiene.spec.ts` passed 61 tests on the rebased branch.
...
- [x] Parent focused tests and docs validation floor after implementation. Evidence: after replaying the CO-387 patch onto current `origin/main`, focused docs-hygiene tests passed 61 tests; `npm run build`, `npm run lint`, `npm run test` passed 4872 tests, `npm run docs:check`, `npm run docs:freshness`, `npm run repo:stewardship`, and diff-budget with explicit override passed.
