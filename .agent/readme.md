...
- **Run diagnostics:** `npx codex-orchestrator start diagnostics --format json --no-interactive` executes the default build/lint/test/spec-guard pipeline and prints the run id plus manifest path under `.runs/<task>/cli/<run-id>/`.
...
- **Legacy shims:** Repo-only MCP wrapper scripts are removed; prefer the `codex-orchestrator` binary for new workflows.
