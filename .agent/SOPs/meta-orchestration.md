...
5. **Delegation is required once a task id exists.** Every top-level task with a task id must have at least one worker run before review gates proceed (use `DELEGATION_GUARD_OVERRIDE_REASON` only when delegation is impossible).
