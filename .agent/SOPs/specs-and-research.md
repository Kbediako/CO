...
- G1 — PRD approval must be recorded in `tasks/index.json` under the task’s `gate` metadata (phase/status/log/run_id) with a safe approval mode run ID before downstream work starts.
