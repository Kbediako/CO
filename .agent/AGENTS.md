<!-- codex:instruction-stamp babb8307ff2ff79a9bfd94bfc1713fc97440cbbe7def794d80a0bd79adab501b -->
# Agent Enablement
...
- Upstream `rust-v0.128.0` removed `js_repl` and `js_repl_tools_only`, and current local `0.135.0` still reports both as removed; do not set `CODEX_CLOUD_ENABLE_FEATURES` / `CODEX_CLOUD_DISABLE_FEATURES` to either `js_repl` or `js_repl_tools_only`, or run `codex features enable/disable js_repl` / `codex features enable/disable js_repl_tools_only`. Use cloud feature env vars only for active non-removed feature names after checking `codex features list`. `goals` is stable on current local `0.135.0`.
