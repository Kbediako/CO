...
- [ ] Parent verifies the archive payload for archived stubs is available on `doc-archives` or records an explicit merge waiver that names the missing payload, owner, expiry, and recovery command.
...
- [ ] Parent records archive payload availability on `doc-archives`, or an explicit payload waiver, before merge.
