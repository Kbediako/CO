...
### Phase 19 Runbook (draft)
1) Remove the persistence `writeAtomicFile` and JSONL writer shims, and inline `WINDOWS_FORBIDDEN_CHARACTERS` in `sanitizeIdentifier`.
2) Run full guardrails (spec-guard → build/lint/test → documentation gate → diff budget → review).

### Phase 20 Runbook (draft)
1) Normalize guardrail summary detection to match spec-guard runner commands across ID/title/command.
2) Remove control-plane + scheduler index shims and update services/tests to import concrete modules.
3) Run full guardrails (spec-guard → build/lint/test → documentation gate → diff budget → review).
...
- [x] Phase 2 consolidations executed (helpers and legacy scripts) - Evidence: `.runs/0101-slimdown-audit/cli/2026-01-01T09-23-12-672Z-1658902e/manifest.json`.
  - [x] Date parsing helper consolidation (reuse parseDateString in parseIsoDate) - Evidence: `.runs/0101-slimdown-audit/cli/2026-01-03T16-07-07-173Z-61412973/manifest.json`.
  - [x] Remove legacy `CODEX_ORCHESTRATOR_REPO_ROOT` fallback in run-manifests resolver - Evidence: `.runs/0101-slimdown-audit/cli/2026-01-03T17-59-01-839Z-0bed253b/manifest.json`.
