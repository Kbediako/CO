...
- [x] Parent validation passes on current `origin/main` merge. Evidence: `node scripts/delegation-guard.mjs`, `node scripts/spec-guard.mjs --dry-run`, `npm run build`, `npm run lint` (3 pre-existing warnings in `orchestrator/tests/DelegationMcpHealth.test.ts`), `npm run test` (352 files / 4762 tests), `npm run docs:check`, `npm run docs:freshness`, `npm run repo:stewardship`, and `node scripts/diff-budget.mjs`.
