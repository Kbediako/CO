...
- [x] CO-521-style Backlog/backlog released `not_active` path performs a bounded direct issue-by-id verification when the current poll snapshot is unavailable, records recent `passive_release` metadata for the same Backlog snapshot, skips repeated direct reads, and starts work if direct truth is active. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderIssueHandoff.test.ts -t "verifies released Backlog not-active claims before making no-poll snapshots passive|revalidates released Backlog not-active claims when no poll snapshot but issue-by-id is active"`.
- [x] CO-520-style stale blocker-snapshot Backlog/backlog released `not_active` path records recent `passive_release` metadata from direct own-issue truth, continues bounded direct revalidation while blocker-mismatched poll evidence persists, and does not start/resume work. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderIssueHandoff.test.ts -t "Backlog not-active claims|blocker snapshots|issue-by-id is active"`.
- [x] PR #862 Codex P1 rework validation: blocker-mismatched poll evidence continues bounded direct revalidation after `passive_release` is recorded instead of using the repeated-read skip. Evidence: focused CO-571 Backlog/blocker/active-direct subset, full `ProviderIssueHandoff.test.ts`, `npm run build`, `git diff --check`, spec guard, `npm run docs:check`, diff budget, `npm run lint`, full `npm run test` (366 files / 6139 tests), `npm run repo:stewardship`, `npm run pack:smoke`, and enforced `gpt-5.5/xhigh` review at `.runs/linear-cdf8b078-95af-4e75-99e2-6c32fa1ecd63-guard/cli/2026-05-21T05-30-22-619Z-f8e4662b/review/output.log`; `npm run docs:freshness` still fails on unrelated CO-558 rolling cohorts (257 stale docs, 199 rolling entries).
...
- [ ] `npm run docs:freshness`. Evidence: current seventh-rework run failed on unrelated CO-558 rolling cohort baseline debt (`257` stale docs, `199` rolling cohort entries); this is not caused by the CO-571 diff.
...
- Steady-state proof: raw released claim rows remain source-labeled audit evidence, while strong terminal released `not_active` claims remain passive under the terminal-claim rules, and passive Backlog released `not_active` claims suppress repeated direct issue-by-id only when recent matching `passive_release` verification exists for the claim's own Backlog snapshot plus complete cached metadata, null retry fields, no active or cancelable retained run, and no blocker-mismatched poll evidence; these rows do not drive `restart_required` or retrying WIP, including when the current poll snapshot is unavailable.
- Tests/docs: `ProviderIssueHandoff.test.ts` terminal released metadata-only table, no-current-poll-snapshot regression, current-poll terminal snapshot regression, map-missing terminal snapshot regression, CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification and active direct issue-by-id regressions, CO-520 stale blocker-snapshot regression, CO-522 stale retained merged-closeout regressions, accepted pending-revalidation no-current-poll regression, stale/current `review_promotion` regressions, active-stuck regression, `ControlRuntime.test.ts` retry projection regression, and this CO-571 packet.
...
- 2026-05-21: PR #860 merged, but live latest-main evidence still looped on CO-521 and then CO-524 Backlog/backlog `claim_issue_by_id:released` when the current poll snapshot was unavailable. CO-571 reopened to Rework, and PR #861 review tightened the fix so no-poll Backlog passive classification requires a recent explicit `passive_release` direct issue-by-id verification and active direct truth still starts work.
- 2026-05-21: PR #861 merged, but live latest-main evidence still looped on CO-520 because stale blocker metadata said Blocked/started while direct issue-by-id verified CO-520 itself was Backlog/backlog. This seventh rework lets direct own-issue Backlog proof mark `passive_release` despite stale blocker snapshots, while blocker-mismatched poll evidence continues bounded direct revalidation and active direct truth still starts work.
