...
- [x] CO-521-style Backlog/backlog released `not_active` path skips direct issue-by-id when the current poll snapshot is unavailable. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderIssueHandoff.test.ts -t "keeps released Backlog not-active claims passive when the current poll snapshot is unavailable"`.
...
- [ ] `npm run docs:freshness`. Evidence: current sixth-rework run failed on unrelated CO-558 rolling cohort baseline debt (`257` stale docs, `199` rolling cohort entries); this is not caused by the CO-571 diff.
...
- 2026-05-21: PR #860 merged, but live latest-main evidence still looped on CO-521 and then CO-524 Backlog/backlog `claim_issue_by_id:released` when the current poll snapshot was unavailable. CO-571 reopened to Rework and the no-poll Backlog classifier gap is now covered by a focused regression.
