...
- Latest-main progress-stall rework: parent-owned Linear/GitHub lifecycle; worker branch only.
...
- [x] `claim_issue_by_id:released_deferred`
- [x] progress-based provider polling stuck deadlines
- [x] non-forcing public lifecycle watchdog re-arm
...
- [x] A progressing passive Backlog refresh uses latest progress, not total operation age, for non-forcing stuck deadlines; real no-progress active refreshes and explicit recovery deadlines still fail closed. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderPollingHealth.test.ts` and `npx vitest run --config vitest.config.core.ts orchestrator/tests/ControlServerPublicLifecycle.test.ts`.
...
- [x] Progress-stall rework full validation completed except unrelated docs freshness baseline. Evidence: focused public lifecycle and provider polling health tests, focused passive Backlog direct-proof slice, `git diff --check`, spec guard dry-run, diff budget, build, lint, full `npm run test` (366 files / 6149 tests), `docs:check`, repo stewardship, pack smoke, manifest-backed diagnostics, and enforced `gpt-5.5/xhigh` review at `.runs/linear-cdf8b078-95af-4e75-99e2-6c32fa1ecd63-validation/cli/2026-05-21T10-40-13-545Z-1393a9bc/review/telemetry.json`.
...
- 2026-05-21: PR #863 merged, but live current-main evidence at `557d5bee30879686e955029c45264950495ef243` still reported `provider_refresh_lifecycle_stuck` / `restart_required` at `claim_issue_by_id:released_deferred`. The progress-stall rework measures stuck deadlines from latest provider polling progress and re-arms non-forcing lifecycle watchdogs after progress, without special-casing the deferred reason string or weakening explicit recovery and real no-progress active stalls.
