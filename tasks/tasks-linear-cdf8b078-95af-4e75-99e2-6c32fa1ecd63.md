...
- [x] CO-520-style stale blocker-snapshot Backlog/backlog released `not_active` path records recent `passive_release` metadata from direct own-issue truth, skips repeated direct reads, and does not start/resume work. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderIssueHandoff.test.ts -t "Backlog not-active claims|blocker snapshots|issue-by-id is active"`.
...
- [x] Seventh-rework focused stale blocker-snapshot regression. Evidence: focused CO-571 Backlog/blocker/active-direct subset passed after allowing direct own-issue Backlog proof to mark and reuse `passive_release` despite stale blocker metadata.
