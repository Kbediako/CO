...
- [x] Large no-poll Backlog released cohorts use a bounded per-refresh direct proof budget, record `claim_issue_by_id:released_deferred` for deferred rows, progress deferred rows over later refreshes, and never start work for verified passive rows. Evidence: `npx vitest run --config vitest.config.core.ts orchestrator/tests/ProviderIssueHandoff.test.ts -t "Backlog not-active claims|large no-poll cohorts|blocker mismatches|issue-by-id is active"` passed.
