...
- Pre-implementation issue-quality review evidence:
  ...
  - the prior closed `CO-132` attempt demonstrates this lane is narrower than a generic test-infra rewrite, but its outcome must now be revalidated on the current baseline
...
- Architecture / design adjustments:
  ...
  - compare the fresh current run against the prior closed `CO-132` attempt to decide whether the lane is still red or already cleared on newer heads
...
- Tests / checks:
  ...
  - capture fresh `npm run test` evidence or an exact environment-sensitive blocker record on `f75e702ea`
