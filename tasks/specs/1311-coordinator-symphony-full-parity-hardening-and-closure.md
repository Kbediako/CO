...
review_notes:
  ...
  - 2026-03-21: March 21 review-fix tranche landed for queued manual refreshes during in-flight provider handoff work, truthful selected-run workspace fallback under overridden runs roots, and real repo-root provider workspace cleanup when `CODEX_ORCHESTRATOR_RUNS_DIR` is external. The same day also landed the manifest-persister force-preempt fix; a trivial `CodexOrchestrator.start()` repro dropped from about `5.1s` to about `112ms`, and local full `npm run test` is terminal again at `282/282` files and `2014/2014` tests in `204.37s`.
---
...
- full repo gate chain, with the explicit current truth that local `MCP_RUNNER_TASK_ID=1311-coordinator-symphony-full-parity-hardening-and-closure npm run test` is terminal again at `282/282` files and `2014/2014` tests in `204.37s`
- `npm run docs:check`
