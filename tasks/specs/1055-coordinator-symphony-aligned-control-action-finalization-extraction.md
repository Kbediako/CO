...
2. Move the remaining finalization logic that:
   ...
   - returns a typed plan containing success response data, audit payload data, the snapshot to finalize, and the already-determined `persistRequired` / `publishRequired` flags.
3. Keep `controlServer.ts` responsible for route ordering, cancel-confirmation authority, transport nonce consume/rollback durability, actual persistence, actual publish, actual audit emission, and raw HTTP writes.
