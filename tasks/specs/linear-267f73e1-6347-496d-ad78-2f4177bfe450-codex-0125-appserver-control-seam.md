...
review_notes:
  ...
  - 2026-04-24: Parent canary evidence holds app-server adoption because sticky environments and resume/fork turn pagination were blocked in the worker canary, while schema, Unix socket/proxy, permission profile, untrusted config, and WebSocket burst checks passed.
---

# Technical Specification

## Context
CO-351 exists to validate whether Codex CLI 0.125.0 exposes an app-server control seam that CO can safely use. Current CO provider-worker posture remains `codex exec / codex exec resume` until a guarded app-server path proves it can preserve authority, permission, thread/config, sticky-environment, pagination, and transport behavior. The packet must make canary evidence and the final hold/adopt decision mandatory; release-note summarization is not enough.
...
5. Require a parent-owned hold/adopt decision that explicitly states whether CO continues to hold on `codex exec / codex exec resume` or adopts a 0.125 app-server seam with documented boundaries.
...
8. Parent adoption decision must remain HOLD unless sticky environment and resume/fork turn-pagination evidence is clean in a configured lane.
...
Wrong interpretations rejected: treating CO-351 as release-note summarization, immediately promoting Codex CLI `0.125.0`, removing `codex exec / codex exec resume` fallback before canary evidence, assuming `unix://` behavior from docs or local help alone, using this child lane to edit code or policy, and broadening into full app-server migration.

Explicit non-goals carried forward: no runtime-provider adoption, no removal of `codex exec / codex exec resume`, no blind promotion of Codex CLI `0.125.0`, no implementation rewrite, and no broad app-server migration in this issue.

## Parity / Alignment Matrix
- Current truth: CO's trusted provider-worker path is still `codex exec / codex exec resume`; app-server is not adopted as the provider control seam.
...
- Target truth / intended delta: parent produces CO-relevant app-server canary evidence and records a hold/adopt decision. Current target decision is HOLD: keep provider workers on `codex exec / codex exec resume` until sticky environment and resume/fork turn-pagination evidence is clean in a configured lane.
...
- Functional requirements:
  ...
  - Make canary-driven hold/adopt acceptance mandatory.
...
- Rollout verification: parent records hold/adopt decision and evidence paths before any version-policy or runtime-seam adoption claim. Current evidence path is `out/linear-267f73e1-6347-496d-ad78-2f4177bfe450/manual/codex-0125-appserver-canary/runtime-canary-summary.json`.
...
- What failure threshold turns partial canary success into hold rather than adopt?
