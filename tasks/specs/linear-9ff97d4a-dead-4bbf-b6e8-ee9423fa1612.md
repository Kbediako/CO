...
review_notes:
  ...
  - 2026-03-30: Pre-implementation review approves the narrow repair path: keep explicit scoped launches promptless, synthesize bounded reviewer-visible scoped title context from resolved `NOTES` plus review surface when `--title` is absent, preserve explicit user titles when present, and update telemetry/docs/tests to make that contract explicit.
