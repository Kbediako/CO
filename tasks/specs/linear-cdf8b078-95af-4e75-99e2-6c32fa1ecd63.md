...
13. Bound unverified no-current-poll Backlog released direct issue-by-id reads per refresh, expose deferred progress as `claim_issue_by_id:released_deferred`, and allow deferred rows to receive direct verification over subsequent refreshes even when earlier exact-snapshot proofs are older than the prior time-based passive-proof window.
