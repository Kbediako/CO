...
| Backlog released not_active | After PR #858 merged, CO-529 Backlog/backlog stayed released `provider_issue_released:not_active` but still reached `claim_issue_by_id:released` on latest main. After PR #860 merged, CO-521 and CO-524 reproduced the same direct issue-by-id loop when the current poll snapshot was unavailable. After PR #861 merged, CO-520 reproduced the loop because a stale blocker snapshot still said Blocked/started while direct issue-by-id verified the claim's own issue was Backlog/backlog. | Backlog is parked work; it should not need repeated direct issue-by-id reads unless active, reopened, retry, pending-review, Blocked, merge-closeout, live-worker, or retained-run evidence requires revalidation. When no current poll snapshot exists or a blocker snapshot disagrees, passive classification must first bind to direct issue-by-id truth for the released claim's own Backlog snapshot. | Passive Backlog released rows with null retry metadata and no active/cancelable run, promotion, or merge-closeout evidence skip repeated direct issue-by-id and do not drive restart-required health only after recent explicit `passive_release` verification; stale blocker snapshots cannot veto that marker, and active direct issue-by-id truth still starts work. | Blanket-suppressing released claims or hiding Blocked/retry/reopened/pending-review/merge-closeout work. |
...
12. Let direct issue-by-id proof for the released claim's own Backlog/backlog state create and reuse `passive_release` even when another current-poll issue carries stale Blocked/started blocker metadata for that claim.
...
- CO-520-style stale blocker-snapshot Backlog history records explicit `passive_release` metadata from direct own-issue Backlog truth, skips repeated direct reads, and starts no work.
