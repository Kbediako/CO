...
| Backlog released not_active | After PR #858 merged, CO-529 Backlog/backlog stayed released `provider_issue_released:not_active` but still reached `claim_issue_by_id:released` on latest main. After PR #860 merged, CO-521 and CO-524 reproduced the same direct issue-by-id loop when the current poll snapshot was unavailable. After PR #861 merged, CO-520 reproduced the loop because a stale blocker snapshot still said Blocked/started while direct issue-by-id verified the claim's own issue was Backlog/backlog. | Backlog is parked work; it should not need repeated direct issue-by-id reads unless active, reopened, retry, pending-review, Blocked, merge-closeout, live-worker, retained-run, or blocker-mismatched evidence requires revalidation. When no current poll snapshot exists or a blocker snapshot disagrees, passive classification must first bind to direct issue-by-id truth for the released claim's own Backlog snapshot. | Passive Backlog released rows with null retry metadata and no active/cancelable run, promotion, or merge-closeout evidence skip repeated direct issue-by-id and do not drive restart-required health only after recent explicit `passive_release` verification and no blocker-mismatched poll evidence; stale blocker snapshots cannot veto marker creation, blocker mismatches still revalidate, and active direct issue-by-id truth still starts work. | Blanket-suppressing released claims or hiding Blocked/retry/reopened/pending-review/merge-closeout work. |
...
11. Keep passive Backlog/backlog released `not_active` rows out of repeated direct issue-by-id when they have no active run, retry, promotion, merge-closeout, cancelable retained-run, or blocker-mismatched poll evidence and a recent explicit `passive_release` direct issue-by-id verification matches the same Backlog snapshot; if no verification exists, perform the bounded direct read and start work when the direct issue truth is active.
12. Let direct issue-by-id proof for the released claim's own Backlog/backlog state create `passive_release` even when another current-poll issue carries stale Blocked/started blocker metadata for that claim, but continue bounded direct revalidation while that blocker mismatch persists.
13. Keep stale retained merged-closeout released rows out of direct issue-by-id when current polling omits them and there is no active run, retry, current promotion, or cancelable retained-run evidence; clear stale `merge_closeout` residue when direct live Done truth is read.
14. Avoid timeout-only changes and provider-intake manual edits.
...
| Passive Backlog released not_active classification | Parked Backlog released claims can escalate to direct issue-by-id stuck refresh health. | remove fallback | CO-571 | CO-529-style released `provider_issue_released:not_active` Backlog/backlog row with no active run, retry, promotion, merge-closeout, or cancelable retained-run evidence, including the CO-521/CO-524 no-current-poll-snapshot recurrence and CO-520 stale blocker-snapshot recurrence. | Observed 2026-05-21 | 2026-05-21 | This issue | Passive Backlog/not_active rows skip repeated direct issue-by-id only after recent explicit `passive_release` direct issue-by-id verification for the claim's own Backlog snapshot and no blocker-mismatched poll evidence, while active/reopened/pending-review/retry/Blocked/merge-closeout paths and blocker mismatches still revalidate. | Focused CO-529 current-poll-omitted, CO-521 no-current-poll Backlog passive-verification, CO-520 stale blocker-snapshot regression, and active direct issue-by-id regressions plus existing active/reopened/current-promotion/active-stall regressions. |
...
- Steady-state proof: raw released claim rows remain source-labeled audit evidence, while terminal released `not_active` claims, passive Backlog released `not_active` claims with a recent matching `passive_release` direct issue-by-id verification and no blocker-mismatched poll evidence, and stale retained merged-closeout rows with complete cached metadata, null retry fields, and no active or cancelable retained run do not drive `restart_required` or retrying WIP.
...
- CO-520-style stale blocker-snapshot Backlog history records explicit `passive_release` metadata from direct own-issue Backlog truth, continues bounded direct revalidation while blocker-mismatched poll evidence persists, and starts no work.
