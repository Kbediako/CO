...
## Summary
- Objective: repair provider refresh lifecycle classification so retained released terminal historical claims, parked Backlog released `not_active` claims, stale retained merged-closeout history, stale blocker snapshots, and large no-poll Backlog released cohorts do not drive `provider_refresh_lifecycle_stuck` / `restart_required` when WIP is `0/3` and no active run/retry/worker corroboration exists.
- Scope:
  ...
  - CO-520-style Backlog/backlog released `not_active` recurrence when stale blocker snapshots disagree with direct issue-by-id truth for the released claim's own issue
  - large no-poll Backlog/backlog released `not_active` cohorts that need bounded direct proof reads and deferred progress over later refreshes
  - PR #863 progress-stall recurrence where `claim_issue_by_id:released_deferred` is legitimate refresh progress but total operation age can still cross the watchdog window
...
| Backlog released not_active | After PR #858 merged, CO-529 Backlog/backlog stayed released `provider_issue_released:not_active` but still reached `claim_issue_by_id:released` on latest main. After PR #860 merged, CO-521 and CO-524 reproduced the same direct issue-by-id loop when the current poll snapshot was unavailable. After PR #861 merged, CO-520 reproduced the loop because a stale blocker snapshot still said Blocked/started while direct issue-by-id verified the claim's own issue was Backlog/backlog. After PR #862 merged, current-source proof still looped through released Backlog no-poll rows because stale blocker metadata could keep verified passive rows in the read path and large cohorts had no proof-read budget. | Backlog is parked work; it should not need repeated direct issue-by-id reads unless active, reopened, retry, pending-review, Blocked, merge-closeout, live-worker, or retained-run evidence requires revalidation. When no current poll snapshot exists or stale blocker metadata disagrees, passive classification must first bind to direct issue-by-id truth for the released claim's own Backlog snapshot. | Passive Backlog released rows with null retry metadata and no active/cancelable run, promotion, or merge-closeout evidence skip repeated direct issue-by-id and do not drive restart-required health only after matching explicit `passive_release` verification; matching direct proof wins over stale blocker metadata, unverified no-poll rows use a per-refresh direct-read budget with deferred progress, and active direct issue-by-id truth still starts work. | Blanket-suppressing released claims, hiding Blocked/retry/reopened/pending-review/merge-closeout work, or deleting deferred claim evidence. |
| Large no-poll Backlog released cohorts | After PR #862 merged, current-source control-host refresh still looped through CO-521 and CO-524 released Backlog rows because direct proof remained serial and stale blocker metadata could keep verified passive rows in the read path. The first eighth-rework review found a time-based proof window could still reread early rows and starve the tail. | Verification should be explicit but bounded: no-poll Backlog rows need direct own-issue proof before passive suppression, and a large cohort should advance over refreshes instead of tripping the lifecycle watchdog. | Matching direct own-issue `passive_release` proof suppresses repeated reads even with stale blocker metadata and remains valid for that exact issue snapshot until `issue_updated_at` changes; unverified no-poll Backlog rows consume a per-refresh direct-read budget, record `claim_issue_by_id:released_deferred` for deferred rows, and progress over subsequent refreshes. | Hiding active direct issue truth or deleting deferred claim evidence. |
| Progressing passive Backlog refresh | After PR #863 merged, the live source was current but health still reported `provider_refresh_lifecycle_stuck` / `restart_required` at `claim_issue_by_id:released_deferred`, a progress marker for bounded passive Backlog proof deferral. | A refresh that continues to record progress should not be judged stuck only by total operation age. | Provider polling health and the public lifecycle watchdog measure stuck deadlines from the latest refresh progress when present; explicit recovery deadlines and true no-progress active refreshes still fail closed. | Special-casing `claim_issue_by_id:released_deferred` as always healthy or hiding real failed refreshes. |
...
- Not done if:
  ...
  - a passive Backlog/backlog released `not_active` row with no active/retry/promotion/merge-closeout evidence is hidden during a no-current-poll refresh without a matching explicit `passive_release` direct issue-by-id verification for the same Backlog snapshot
  - a stale blocker snapshot can suppress `passive_release` after direct issue-by-id verifies the released claim's own issue is Backlog/backlog
  - matching direct own-issue `passive_release` proof is ignored because stale blocker metadata still forces repeated direct issue-by-id reads
  - a large no-poll Backlog cohort exhausts the refresh lifecycle instead of using a bounded direct-read budget and deferred progress
  - deferred `claim_issue_by_id:released_deferred` rows never progress to direct verification in later refreshes
  - a progressing refresh with recent `claim_issue_by_id:released_deferred` progress is marked stuck solely because total operation age crossed the watchdog window
  - explicit recovery deadlines or real no-progress active refreshes stop returning `provider_refresh_lifecycle_stuck` / `restart_required`
...
- Pre-implementation issue-quality review evidence:
  ...
  - 2026-05-21 sixth rework: PR #860's merged-closeout fix was partial. Live latest-main evidence at `65b05d2525a6c0fe793bf778d6f42147bc75aceb` still looped through `claim_issue_by_id:released` for CO-521 and then CO-524, both Backlog/backlog released `provider_issue_released:not_active` rows with no retained run, retry, review-promotion, or merge-closeout metadata. The Backlog passive classifier must apply when `resolveTrackedIssues` skips only after a direct issue-by-id read verifies the same inert Backlog snapshot and stores an explicit `passive_release` marker.
  - 2026-05-21 seventh rework: PR #861's no-poll Backlog fix was partial. Live latest-main evidence at `c66abd777d74aebbf9f8b046f54581962ad7703a` still looped through `claim_issue_by_id:released` for CO-520 because stale blocker metadata said Blocked/started while direct issue-by-id verified CO-520 itself was Backlog/backlog. Direct own-issue verification must outrank stale blocker snapshots for `passive_release`, while active direct truth still starts work.
  - 2026-05-21 eighth rework: PR #862's blocker-snapshot fix was partial. Live current-source evidence at `afc33fe2ebbb24b62f44dc7fc7811ad0aba28029` still looped through released Backlog no-poll claim reconciliation for CO-521 and then CO-524. Matching direct own-issue `passive_release` must suppress repeated reads even when stale blocker metadata persists, unverified large no-poll Backlog cohorts must use bounded per-refresh direct proof with `claim_issue_by_id:released_deferred` progress that cannot be starved by time-only expiry of earlier exact-snapshot proofs, and stale exact-snapshot proof must later revalidate so active direct truth cannot remain hidden while polling is unavailable.
  - 2026-05-21 progress-stall rework: PR #863's passive-proof budget fix was partial. Live current-main evidence at `557d5bee30879686e955029c45264950495ef243` still reported `provider_refresh_lifecycle_stuck` / `restart_required` at `claim_issue_by_id:released_deferred`. The watchdog must measure from latest provider polling progress for non-forcing refreshes, while explicit recovery deadlines and true no-progress active stalls still fail closed.
...
11. Keep passive Backlog/backlog released `not_active` rows out of repeated direct issue-by-id when they have no active run, retry, promotion, merge-closeout, or cancelable retained-run and a matching explicit `passive_release` direct issue-by-id verification matches the same Backlog snapshot; if no verification exists, perform a bounded direct read and start work when the direct issue truth is active.
12. Let direct issue-by-id proof for the released claim's own Backlog/backlog state create `passive_release` even when another current-poll issue carries stale Blocked/started blocker metadata for that claim, and let that matching direct proof suppress repeated reads despite the stale blocker metadata.
13. Bound unverified no-current-poll Backlog released direct issue-by-id reads per refresh, expose deferred progress as `claim_issue_by_id:released_deferred`, allow deferred rows to receive direct verification over subsequent refreshes even when earlier exact-snapshot proofs are older than the prior time-based passive-proof window, and revalidate stale exact-snapshot proof after unverified rows have priority.
14. Measure refresh stuck deadlines from the latest provider polling progress when progress exists, re-arm non-forcing public lifecycle watchdogs after progress, and preserve forced recovery deadlines plus true no-progress active stuck behavior.
15. Keep stale retained merged-closeout released rows out of direct issue-by-id when current polling omits them and there is no active run, retry, current promotion, or cancelable retained-run evidence; clear stale `merge_closeout` residue when direct live Done truth is read.
16. Avoid timeout-only changes and provider-intake manual edits.
...
- Steady-state proof: raw released claim rows remain source-labeled audit evidence, while terminal released `not_active` claims, passive Backlog released `not_active` claims with a matching `passive_release` direct issue-by-id verification for the same issue snapshot, and stale retained merged-closeout rows with complete cached metadata, null retry fields, and no active or cancelable retained run do not drive `restart_required` or retrying WIP. Unverified no-poll Backlog rows are direct-read within a per-refresh budget and deferred rows remain visible until subsequent refreshes verify them.
- Tests/docs: `ProviderIssueHandoff.test.ts` terminal released metadata-only table, no-current-poll-snapshot regression, current-poll terminal snapshot regression, map-missing terminal snapshot regression, CO-529 Backlog regression, CO-521 no-current-poll Backlog passive-verification and active direct issue-by-id regressions, CO-520 stale blocker-snapshot regression, large no-poll Backlog cohort budgeting regression, stale exact-snapshot proof active revalidation regression, `ProviderPollingHealth.test.ts` progress-clock and no-progress stuck regressions, `ControlServerPublicLifecycle.test.ts` progress-based watchdog re-arm regression, CO-522 stale retained merged-closeout regressions, stale/current `review_promotion` regressions, active-stuck regression, `ControlRuntime.test.ts` retry projection regression, and this CO-571 packet.
...
- CO-520-style stale blocker-snapshot Backlog history records explicit `passive_release` metadata from direct own-issue Backlog truth, matching direct proof suppresses repeated reads despite stale blocker metadata, and starts no work.
- Large no-poll Backlog released cohorts use a bounded per-refresh direct proof budget, record `claim_issue_by_id:released_deferred` for deferred rows, progress deferred rows over later refreshes even after time advances beyond the prior passive-proof window, and never start work for verified passive rows.
- A progressing passive Backlog refresh uses latest progress, not total operation age, for non-forcing stuck deadlines; real no-progress active refreshes and explicit recovery deadlines still fail closed.
...
- Stale blocker-snapshot Backlog passive-verification regression.
- Large no-poll Backlog direct-read budget and deferred-progress regression.
- Provider polling progress-clock and public lifecycle watchdog re-arm regressions.
