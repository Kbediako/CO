...
14. Keep stale retained merged-closeout released rows out of direct issue-by-id when current polling omits them and there is no active run, retry, current promotion, or cancelable retained-run evidence; clear stale `merge_closeout` residue when direct live Done truth is read.
15. Avoid timeout-only changes and provider-intake manual edits.
