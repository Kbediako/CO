...
- Shared source note: expected shared source text is absent in this child checkout; the packet therefore preserves only the protected issue wording supplied in the bounded handoff plus current repo seam names.

## Summary
- Objective: stop false synthesis of `Guardrails: spec-guard command not found.` when no `spec-guard` stage exists, and make manifest summaries, metrics closeout, provider issue observability, and retry truth consume the same applicability contract.
- Scope:
  - docs-first packet and registry/checklist mirrors
  - one parent-owned shared applicability implementation
  - parent-owned focused validation only
- Constraints:
  - preserve exact issue wording
  - preserve real `spec-guard` outcomes when a stage exists
  - do not widen into generic guardrail redesign, unrelated scheduler/provider changes, or docs-only reinterpretation

## Issue-Shaping Contract
- User-request translation carried forward: this lane is about false runtime synthesis of a non-existent `spec-guard` failure across multiple truth surfaces, not about broad guardrail or provider redesign.
...
- Nearby wrong interpretations to reject:
  ...
  - unrelated scheduler/provider lifecycle changes
  - docs-only reinterpretation or waiver
  - suppressing all missing-guardrail states instead of distinguishing applicability
  - letting each surface keep its own local applicability rule
- Explicit non-goals carried forward:
  - no stage-set overhaul
  - no scheduler redesign
  - no unrelated provider-worker retry or merge-closeout work
  - no docs-only reinterpretation
  - no code or test edits in this child lane

## Parity / Alignment Matrix
- Current truth:
  - manifest guardrail summary treats `counts.total === 0` as `Guardrails: spec-guard command not found.`
  - metrics closeout can preserve or append that fake summary into otherwise truthful terminal provider-worker closeout
  - provider issue observability and retry truth can inherit or consume the fake summary as if a real guardrail failure occurred
- Reference truth:
  - no `spec-guard` stage should mean `not applicable`, not `missing`
  - only a real stage or explicit required-missing condition should generate missing/failure truth
  - manifest, metrics, provider issue observability, and retry truth should use the same structured applicability state
- Target truth / intended delta:
  - one shared applicability contract distinguishes `not applicable`, `required missing`, and real present-stage outcomes
  - false runtime synthesis disappears from manifest summaries and metrics closeout
  - observability and retry truth consume the same applicability state instead of raw synthesized summary text
...
- Not done if:
  ...
  - metrics closeout can still preserve or append fake missing-guardrail summaries for non-applicable runs
  - provider issue observability or retry truth can still treat the fake summary as real failure truth
...
- Pre-implementation issue-quality review evidence:
  - 2026-04-18: the bounded handoff wording makes `CO-225` a multi-surface applicability / truth contract, not a narrower manifest-only cleanup or a docs-only classification lane. The packet therefore preserves exact wording across manifest summaries, metrics closeout, provider issue observability, and retry truth.
  - 2026-04-18: the micro-task path is ineligible because correctness depends on exact wording, explicit non-goals, explicit not-done-if conditions, and the shared parity contract before implementation starts.
...
- Functional requirements:
  ...
  4. Ensure metrics closeout does not preserve or append the fake missing-guardrail summary for non-applicable runs.
  5. Ensure provider issue observability and retry truth use the same applicability contract rather than raw synthesized summary text.
...
- Non-functional requirements:
  ...
  - one shared seam, not multiple ad hoc overrides
...
- Architecture / design adjustments:
  - introduce or reuse one shared guardrail-applicability contract such as `not_applicable`, `required_missing`, and `present_*`, or an equivalent structured state
  - keep one helper or one structured `guardrail_status` path reused by manifest summaries, metrics closeout, provider issue observability, and retry truth
  - distinguish command-discovery absence from true required-missing state; substring discovery alone cannot imply missing-guardrail failure
- Required artifact/content expectations:
  ...
  - provider issue observability and retry truth do not project a non-existent `spec-guard` failure as current operator truth
...
- Prefer one bounded applicability contract over multiple ad hoc overrides.
