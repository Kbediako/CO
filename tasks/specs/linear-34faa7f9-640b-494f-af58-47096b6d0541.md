...
## 2026-05-25 Review
- CO-585 pre-expiry review checked the retained released/not_active contract against current source/test anchors in `orchestrator/src/cli/control/providerIssueHandoff.ts` and the retained released metadata cluster in `orchestrator/tests/ProviderIssueHandoff.test.ts`. The spec remains accurate and active; no content change beyond truthful review metadata is required.
