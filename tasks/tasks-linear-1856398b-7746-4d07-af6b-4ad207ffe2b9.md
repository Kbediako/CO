...
- [x] Parent adds focused docs-hygiene tests for stale active release evidence residue, including the `0.124` book-style case. Evidence: `npx vitest run --config vitest.config.core.ts tests/docs-hygiene.spec.ts` passed 50 tests on the rebased branch.
