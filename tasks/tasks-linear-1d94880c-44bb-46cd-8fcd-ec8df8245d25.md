...
## Validation
- [x] Sanitized healthy/degraded fixtures include stale refresh, active manifest with stale proof, terminal proof with active claim, low Linear headroom, stale retry queue, child-lane cap pressure, stale refresh with a recent intake write, nested status Linear budget, retry claims tied to terminal failed runs, stale last-success with failed polling completion, missing active worker proof evidence, worker-audit-only auxiliary evidence, and timestamp-less artifact tie-breaks.
- [x] Focused parser/evaluator/CLI tests pass. Evidence: `npx vitest run orchestrator/tests/ProviderControlHostFreshnessGauge.test.ts` passed 30 tests; strict CLI healthy exit `0`; strict CLI contradictory exit `1`.
...
- [x] Final PR feedback cleanup passed local validation. Evidence: CodeRabbit `readState` simplification applied without behavior change; latest Codex P2 feedback fixed by requiring at least one core freshness source before a healthy verdict and by selecting timestamp-less artifacts deterministically by path; invalid `--now` strictness was already present and covered; `npx vitest run orchestrator/tests/ProviderControlHostFreshnessGauge.test.ts` (30 tests), `npx tsc -p tsconfig.build.json --noEmit --pretty false`, `npm run build`, `npm run lint`, and `npm run test` (340 files / 3851 tests) passed.
