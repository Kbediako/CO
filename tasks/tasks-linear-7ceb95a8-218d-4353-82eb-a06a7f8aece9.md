...
## Validation
- [x] Read-only issue-context captured for `CO-303` and `CO-295`. Evidence: `node dist/bin/codex-orchestrator.js linear issue-context --issue-id CO-303 --format json` and `node dist/bin/codex-orchestrator.js linear issue-context --issue-id CO-295 --format json`.
