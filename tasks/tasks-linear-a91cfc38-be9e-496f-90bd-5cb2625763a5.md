...
- 2026-05-24: Codex current-head review found terminal rehydrate side effects could be skipped after retained claim identity drift and stale-run fresh-launch failures could drop retry scheduling. Fixed by decoupling terminal cancellation/cleanup from persisted claim identity while keeping released claim retention identity-aware, and by separating stale identity-cleared launch inputs from retry-state inputs used only for launch-failure requeue.
