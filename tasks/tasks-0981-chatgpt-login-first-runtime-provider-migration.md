...
- [x] W6 default-flip decision recorded with parity/canary evidence and rollback path. - Decision: initial 0981 hold kept default `runtimeMode=cli`; superseded by 0983 canary automation evidence (`out/0983-runtime-default-flip-readiness-automation/manual/runtime-canary-summary.json`) that flipped default to `runtimeMode=appserver` while preserving break-glass `--runtime-mode cli`.
