...
- [ ] docs-review approval captured. Evidence: pending.
...
- [ ] Manifest-backed standalone review executed or truthful fallback recorded. Evidence: non-interactive prompt-only `npm run review -- --commit HEAD` succeeded only without the task manifest context; task-scoped rerun failed with `No run manifests found`, so this remains pending for PR/review lane.
