...
- [x] `BASE_SHA=origin/main DIFF_BUDGET_OVERRIDE_REASON=... node scripts/diff-budget.mjs`. Evidence: exceeded 2129/1200 lines and passed with `DIFF_BUDGET_OVERRIDE_REASON=CO-543 must atomically reclassify 13 terminal stale specs plus explicit nonstandard packet registry rows, spec pre-expiry regression coverage, mixed legacy task-index lifecycle parity, and required fallback-decision evidence so completed packets stop recurring as active freshness debt.`
...
- 2026-05-17: Addressed current-head Codex PR review finding by merging canonical `items[]` and legacy `tasks[]` rows instead of treating them as mutually exclusive; mixed-index focused docs-freshness coverage passes 82 tests.
