...
- [ ] Latest `origin/main` merge refresh plus the post-push `pr ready-review` rerun complete before any review-state transition. Current blocker: after a docs-only handoff-readiness update, GitHub now reports `mergeStateStatus=DIRTY` / `mergeable=CONFLICTING`; the parent lane is resolving the `docs/TASKS.md` and `tasks/index.json` conflict, then it must push the refreshed branch, wait for PR checks to return green, and rerun `codex-orchestrator pr ready-review --pr 537 --quiet-minutes 10 --timeout-minutes 20` before moving the issue to `In Review`.
