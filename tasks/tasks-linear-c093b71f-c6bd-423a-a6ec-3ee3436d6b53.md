...
- [x] Parent adds focused deterministic fixtures. Evidence: `orchestrator/tests/QuotaHygieneCliShell.test.ts` now has 11 deterministic tests for stale intake, app-managed delegates, unloaded launchd, active automation, zero-model/unknown goals, degraded co-status reads, selected-claim self-corroboration rejection, provider runner process detection, canonical provider-intake active semantics, uppercase Codex app processes, and `ps` parsing.
